...
`).trim()}}function pr(e){const n=e.replace(/\.md$/,"").replace(/(^|\/)index$/,"");return n===""?"/":`/${n}/`}function gr(e){if(e==="/")return null;const t=e.slice(0,-1);return t.slice(0,t.lastIndexOf("/")+1)}function Yt(e){if(e==="/")return"/";const t=e.slice(0,-1);return t.slice(t.lastIndexOf("/")+1)}function yr(e){const{fields:t,body:n}=fr(e.markdown),a=pr(e.file);return{file:e.file,route:a,parent:gr(a),name:Yt(a),title:t.title??Yt(a),summary:t.summary??"",order:Number(t.order??"100"),body:n,fields:t}}function Ut(e,t){return e.order-t.order||e.name.localeCompare(t.name)}class wr{byRoute;constructor(t){const n=t.map(yr).sort(Ut);this.byRoute=new Map(n.map(a=>[a.route,a]))}get pages(){return[...this.byRoute.values()]}at(t){return this.byRoute.get(t)}childrenOf(t){return this.pages.filter(n=>n.parent===t).sort(Ut)}trailTo(t){const n=this.at(t);return n?n.parent===null?[n]:[...this.trailTo(n.parent),n]:[]}}const Fe=new wr(ur);function Kt(){const e=[...Sn,...re.flatMap(h=>h.commands??[])],t=Object.assign({},...re.map(h=>h.apps??{})),a=(h=>h.endsWith("/")?h:`${h}/`)(window.location.pathname),o=Fe.at(a);let s=Wt(t),r=null;const i=Us(Fe,(h,u)=>{s(),s=Wt(t);for(const f of re)f.arrive?.(h);u||r?.moveTo(h.route)});if(r=dr(Fe,o?a:"/",{moveTo:h=>i(h,{keep:!0}),clearPage:()=>{s(),s=()=>{},document.querySelector("main")?.replaceChildren()},commands:e}),o)for(const h of re)h.arrive?.(o);const l={run:h=>r?.run(h)};for(const h of re)h.install?.(l)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Kt):Kt();
