...
`);return`<pre><code>${_(t)}</code></pre>`}function mn(e,t){const n=[];for(const o of e)t.test(o)?n.push(o.replace(t,"")):n.length&&(n[n.length-1]+=`
...
`)}function $n(e,t){if(t.parent===null)return"";const n=e.childrenOf(t.route);return n.length===0?"":`<ul class="listing">${n.map(a=>`<li><a href="${a.route}">${_(a.title)}</a>`+(a.summary?` <span class="summary">${_(a.summary)}</span>`:"")+"</li>").join("")}</ul>`}function Tn(e,t){const n=e.trailTo(t.route);if(n.length<=1)return"";const o=n.slice(1).map(a=>a.name).join("/");return`<p class="ran"><span class="ps1">~ $</span> cd ${_(o)} &amp;&amp; cat *</p>`}function Sn(e,t){return`${Tn(e,t)}
${nt(t.body)}
...
`)}}};function Yn(e){const t=e.filter(o=>o.startsWith("-")).flatMap(o=>o.slice(1).split("")),n=e.find(o=>!o.startsWith("-"))??".";return{flags:t,path:n}}function jn(e,t,n){const o=n==="."?"":`${n.replace(/\/$/,"")}/`;return[{mode:"--r-",name:"README.md",title:e.title,href:e.route,run:`cat ${o}README.md`},...t.map(a=>({mode:"dr-x",name:`${a.name}/`,title:a.title,href:a.route}))]}function Ye(e){const t=e.run?` data-run="${_(e.run)}"`:"";return`<a href="${_(e.href)}"${t}>${_(e.name)}</a>`}function Bn(e,t){const n=(s,r)=>t?`${s.mode}  ${r.padEnd(20)}  ${s.title}`:r,o=s=>t?`${s.mode}  ${Ye(s)}${" ".repeat(Math.max(0,20-s.name.length))}  ${_(s.title)}`:Ye(s),a=t?[`total ${e.length}`]:[];return{text:[...a,...e.map(s=>n(s,s.name))].join(`
