...
`);return{columns:Math.max(...t.map(n=>n.length)),rows:t.length}}function xt(e,t){lt.get(e)?.();const n=t??{...kt,seed:Math.floor(Math.random()*16777215)},{columns:a,rows:o}=fr(e),s=Math.min(a,o*2),r=ha(n),i=new Et(s);e.dataset.seed=String(n.seed),e.title=`World ${n.seed}, ${r.mesh.faceCount.toLocaleString("en")} triangles`;const h=v=>{e.innerHTML=dr(lr(i.paint(r,{rotation:v}),s)),e.classList.add("grown")};if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return h(.6),xn(r,.6),lt.set(e,()=>{}),()=>{};let c=0,l=-1/0,d=-1/0;const m=performance.now(),f=ca(e),u=v=>{const y=(v-m)/1e3/ur*Math.PI*2;f.onScreen()&&v-l>=mr&&(h(y),l=v),v-d>pr&&(xn(r,y),d=v),c=requestAnimationFrame(u)};c=requestAnimationFrame(u);const g=()=>{cancelAnimationFrame(c),f.stop()};return lt.set(e,g),g}function gr(){const e=document.querySelector(".planet");return e?xt(e,ta()??void 0):()=>{}}const ne=360,wr=60,yr=1.4,$n=Math.PI*2/wr,Tn=Math.PI*4;function br(e){const t=p("canvas",{class:"world",width:ne,height:ne}),n=t.getContext("2d");if(!n)return()=>{};const a={...kt,seed:Math.floor(Math.random()*16777215)},o=n.createImageData(ne,ne),s=new Et(ne,o.data),r=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let i,h=.6,c=-.38,l=!r,d=null,m=0,f=performance.now();const u=p("p",{class:"hint"}),g=document.querySelector(".planet"),v=(20*4**kt.levels).toLocaleString("en"),y=()=>{i=ha(a);const x=ta();u.textContent=`World ${a.seed}: ${i.mesh.faceCount.toLocaleString("en")} triangles. `+(x?`The header is keeping world ${x.seed}, ${(20*4**x.levels).toLocaleString("en")} triangles.`:`The header grows a new one every visit, ${v} triangles each.`),w.hidden=!x,S()},k=p("button",{type:"button",onclick:()=>{Ls({...a}),g&&xt(g,{...a}),y()}},"Put it in the header"),w=p("button",{type:"button",hidden:!0,onclick:()=>{Fs(),g&&xt(g),y()}},"Let the header grow its own"),S=()=>{s.paint(i,{rotation:h,tilt:c}),n.putImageData(o,0,0)};let M=0;const A=ca(t),$=x=>{const j=Math.min(.1,(x-f)/1e3);if(!d&&A.onScreen()){if(m!==0){m*=Math.exp(-j/yr);const b=l?$n:0;(Math.abs(m)<=b||Math.abs(m)<.01)&&(m=0)}m!==0?(h-=m*j,S()):l&&(h+=$n*j,S()),wt.send({byRadians:m*j,tiltedBy:0,seconds:j})}f=x,M=requestAnimationFrame($)};t.addEventListener("pointerdown",x=>{d={x:x.clientX,y:x.clientY,at:x.timeStamp},m=0,t.setPointerCapture(x.pointerId)}),t.addEventListener("pointermove",x=>{if(!d)return;const j=t.clientWidth||ne,b=(x.clientX-d.x)/j*Math.PI;h-=b;const T=c;c=Math.max(-1.2,Math.min(1.2,c-(x.clientY-d.y)/j*Math.PI)),wt.send({byRadians:b,tiltedBy:c-T,seconds:0});const D=Math.max(.004,(x.timeStamp-d.at)/1e3);m=Math.max(-Tn,Math.min(Tn,m*.4+b/D*.6)),d={x:x.clientX,y:x.clientY,at:x.timeStamp},S()}),t.addEventListener("pointerup",x=>{d&&x.timeStamp-d.at>120&&(m=0),d=null,f=performance.now()}),t.addEventListener("pointercancel",()=>{d=null,m=0});const E=p("input",{type:"number",min:0,value:a.seed,onchange:()=>{a.seed=Math.max(0,Math.floor(Number(E.value)||0)),y()}}),P=p("button",{type:"button",onclick:()=>{a.seed=Math.floor(Math.random()*16777215),E.value=String(a.seed),y()}},"Another world"),O=p("button",{type:"button",onclick:()=>{l=!l,O.textContent=l?"Hold still":"Turn"}},l?"Hold still":"Turn"),I=(x,j,b,T,D,N)=>{const C=p("output",{},N(a[x])),L=p("input",{type:"range",min:b,max:T,step:D,value:a[x],onchange:()=>{a[x]=Number(L.value),C.textContent=N(a[x]),y()},oninput:()=>{C.textContent=N(Number(L.value))}});return p("label",{},`${j}: `,C,L)};return e.append(t,p("div",{class:"row"},p("span",{},"Seed "),E,P,O,k,w),p("div",{class:"dials"},I("levels","Detail",2,6,1,x=>`${x} splits`),I("roughness","Roughness",.02,1,.01,x=>x.toFixed(2)),I("share","Sea",0,.98,.01,x=>`${Math.round(x*100)}%`)),u),y(),M=requestAnimationFrame($),()=>{cancelAnimationFrame(M),A.stop()}}const vr={name:"world",apps:{worlds:br},install:()=>gr()},fe=[vr,ss,Uo,Vo,eo,mo,Ka,Ns,xo,Bo,Mo,cs];function Sn(e,t){const n=[];for(const a of document.querySelectorAll(".app[data-app]")){const o=e[a.dataset.app??""]?.(a,t);o&&n.push(o)}return()=>{for(const a of n)a()}}function kr(e){const t={},n=e.fields.theme;(n==="dark"||n==="light")&&(t["data-page-theme"]=n);const a=e.fields.sky;return a&&(t["data-sky"]=a),t}const xr=["data-page-theme","data-sky"];function $r(e,t){return t==="/"?e==="/":e.startsWith(t)}const la=7.8,Mn=17,da=12,Tr=8,dt=28,An=44,ve=8,Sr=40,Mr=16;function Ar(e){const t=new Map;for(const w of e.nodes){const S=w.label.split(`
`),M=Math.max(...S.map(A=>A.length),1);t.set(w.id,{id:w.id,label:w.label,real:!0,rank:-1,along:Math.max(40,M*la+da*2),across:S.length*Mn+Tr*2,pos:0,preds:[],succs:[]})}for(const w of e.edges)if(!t.has(w.from)||!t.has(w.to))throw new Error(`flow: edge ${w.from} --> ${w.to} names a node that is not there`);const n=Ir(e),a={...e,edges:e.edges.map((w,S)=>n.has(S)?{...w,from:w.to,to:w.from}:w)};for(const w of a.edges){const S=t.get(w.from),M=t.get(w.to);S.succs.push(M),M.preds.push(S)}Er(t);const o=Cr(t,a),s=Pr(t);jr(s);const r=s.length,i=s.map(w=>Math.max(Mn,...w.map(S=>S.real?S.across:0))),h=[];let c=ve;for(let w=0;w<r;w+=1)h.push(c),c+=(i[w]??0)+An;const l=w=>(h[w.rank]??0)+((i[w.rank]??0)-(w.real?w.across:0))/2,d=Math.max(...[...t.values()].map(w=>w.pos+w.along))+ve,m=c-An+ve,f=e.direction==="LR",u=(w,S)=>f?[S,w]:[w,S],g=[...t.values()].filter(w=>w.real).map(w=>{const[S,M]=u(w.pos,l(w));return{id:w.id,label:w.label,x:S,y:M,width:f?w.across:w.along,height:f?w.along:w.across}}),v=e.edges.map((w,S)=>{const M=o[S]??[],A=M[0],$=M[M.length-1];if(!A||!$)throw new Error("flow: an edge lost its ends");const E=e.edges.some(j=>j.from===w.to&&j.to===w.from),P=Math.min(Sr,A.along/3,$.along/3),O=E?n.has(S)?P:-P:0,I=[u(A.pos+A.along/2+O,l(A)+A.across),...M.slice(1,-1).map(j=>u(j.pos+j.along/2,l(j)+(i[j.rank]??0)/2)),u($.pos+$.along/2+O,l($))],x=n.has(S)?I.reverse():I;return w.label===void 0?{from:w.from,to:w.to,points:x}:{from:w.from,to:w.to,label:w.label,points:x}}),[y,k]=u(d,m);return{direction:e.direction,width:y,height:k,nodes:g,edges:v}}function Ir(e){const t=new Set,n=new Map,a=o=>{n.set(o,"walking"),e.edges.forEach((s,r)=>{s.from!==o||t.has(r)||(n.get(s.to)==="walking"?t.add(r):n.has(s.to)||a(s.to))}),n.set(o,"done")};for(const o of e.nodes)n.has(o.id)||a(o.id);return t}function Er(e){const t=new Set,n=a=>{if(a.rank>=0)return a.rank;if(t.has(a))throw new Error(`flow: there is a cycle through ${a.id}, and a flow has a direction`);return t.add(a),a.rank=a.preds.length===0?0:Math.max(...a.preds.map(n))+1,t.delete(a),a.rank};for(const a of e.values())n(a)}function Cr(e,t){let n=0;return t.edges.map(a=>{const o=e.get(a.from),s=e.get(a.to);if(!o||!s)return[];const r=[o];let i=o;for(let h=o.rank+1;h<s.rank;h+=1){n+=1;const c={id:`\0${n}`,label:"",real:!1,rank:h,along:Math.max(Mr,(a.label?.length??0)*la+da),across:0,pos:0,preds:[i],succs:[]};e.set(c.id,c),i.succs.push(c),r.push(c),i=c}return i!==o&&(i.succs.push(s),s.preds.push(i),o.succs.splice(o.succs.indexOf(s),1),s.preds.splice(s.preds.indexOf(o),1)),r.push(s),r})}function Pr(e){const t=Math.max(...[...e.values()].map(r=>r.rank))+1,n=Array.from({length:t},()=>[]);for(const r of e.values())n[r.rank]?.push(r);const a=new Map,o=r=>r.forEach((i,h)=>a.set(i,h));n.forEach(o);const s=(r,i)=>i.length===0?a.get(r)??0:i.reduce((h,c)=>h+(a.get(c)??0),0)/i.length;for(let r=0;r<4;r+=1){for(let i=1;i<t;i+=1){const h=n[i]??[];h.sort((c,l)=>s(c,c.preds)-s(l,l.preds)),o(h)}for(let i=t-2;i>=0;i-=1){const h=n[i]??[];h.sort((c,l)=>s(c,c.succs)-s(l,l.succs)),o(h)}}return n}function jr(e){const t=r=>r.reduce((i,h)=>i+h.along,0)+dt*Math.max(0,r.length-1),n=Math.max(...e.map(t));for(const r of e){let i=ve+(n-t(r))/2;for(const h of r)h.pos=i,i+=h.along+dt}const a=r=>r.pos+r.along/2,o=(r,i)=>{for(const h of r){const c=i(h);if(c.length===0)continue;const l=c.reduce((d,m)=>d+a(m),0)/c.length;h.pos=l-h.along/2}for(let h=1;h<r.length;h+=1){const c=r[h-1],l=r[h];if(!c||!l)continue;const d=c.pos+c.along+dt;l.pos<d&&(l.pos=d)}};for(let r=0;r<3;r+=1){for(let i=1;i<e.length;i+=1)o(e[i]??[],h=>h.preds);for(let i=e.length-2;i>=0;i-=1)o(e[i]??[],h=>h.succs)}const s=Math.min(...e.flat().map(r=>r.pos));for(const r of e.flat())r.pos+=ve-s}const $t=/(\w[\w.-]*)(?:\[([^\]]*)\])?/,Or=new RegExp(`^${$t.source}\\s*-->(?:\\|([^|]*)\\|)?\\s*${$t.source}$`),Dr=new RegExp(`^${$t.source}$`),Rr=/^(?:flow\s+)?(TD|LR)$/i;function Nr(e){const t=new Map,n=[];let a="TD";const o=(i,h)=>{i&&(t.has(i)||t.set(i,i),h!==void 0&&t.set(i,h.replace(/\\n/g,`
`)))},s=e.split(`
`);let r=!0;return s.forEach((i,h)=>{const c=i.trim();if(c===""||c.startsWith("%"))return;if(r){r=!1;const m=Rr.exec(c);if(m){a=m[1]?.toUpperCase()==="LR"?"LR":"TD";return}}const l=Or.exec(c);if(l){const[,m,f,u,g,v]=l;o(m,f),o(g,v),n.push(u===void 0?{from:m??"",to:g??""}:{from:m??"",to:g??"",label:u});return}const d=Dr.exec(c);if(d){o(d[1],d[2]);return}throw new Error(`flow: cannot read line ${h+1}: "${c}"`)}),{direction:a,nodes:[...t].map(([i,h])=>({id:i,label:h})),edges:n}}const Lr=20,In=17;function Fr(e){let t=5381;for(let n=0;n<e.length;n+=1)t=(t*33^e.charCodeAt(n))>>>0;return t.toString(36)}const B=e=>String(Math.round(e*10)/10);function Wr(e,t){const[n,...a]=e.points;if(!n)return"";let o=`M${B(n[0])},${B(n[1])}`,s=n;for(const r of a){const[i,h]=s,[c,l]=r,d=t?[(i+c)/2,h]:[i,(h+l)/2],m=t?[(i+c)/2,l]:[c,(h+l)/2];o+=` C${B(d[0])},${B(d[1])} ${B(m[0])},${B(m[1])} ${B(c)},${B(l)}`,s=r}return o}function Br(e){const{points:t}=e,n=t[Math.floor((t.length-1)/2)]??[0,0],a=t[Math.ceil((t.length-1)/2)]??n;return[(n[0]+a[0])/2,(n[1]+a[1])/2]}function Hr(e){const t=Ar(Nr(e)),n=t.direction==="LR",a=`arrow-${Fr(e)}`,o=t.edges.map(h=>{const c=`<path class="edge" d="${Wr(h,n)}" marker-end="url(#${a})"/>`;if(h.label===void 0)return c;const[l,d]=Br(h);return`${c}<text class="edge-label" x="${B(l)}" y="${B(d)}" text-anchor="middle" dominant-baseline="middle">${R(h.label)}</text>`}).join(""),s=t.nodes.map(h=>{const c=h.x+h.width/2,l=h.label.split(`
`),d=h.y+(h.height-l.length*In)/2,m=l.map((f,u)=>`<tspan x="${B(c)}" y="${B(d+Lr-8+u*In)}">${R(f)}</tspan>`).join("");return`<g class="node"><rect x="${B(h.x)}" y="${B(h.y)}" width="${B(h.width)}" height="${B(h.height)}" rx="4"/><text text-anchor="middle" dominant-baseline="middle">${m}</text></g>`}).join(""),r=B(t.width),i=B(t.height);return`<figure class="flow"><svg class="flow" viewBox="0 0 ${r} ${i}" width="${r}" height="${i}" style="max-width: 100%; height: auto" role="img"><defs><marker id="${a}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z"/></marker></defs>${o}${s}</svg></figure>`}const Gr=new Set("var let const function return if else for while do break continue new this true false null undefined class extends import export from default async await throw try catch finally typeof instanceof in of switch case delete void yield".split(" "));function Y(e,t){return`<span class="hl-${e}">${R(t)}</span>`}function ua(e,t,n){for(let a=t+1;a<e.length;a+=1)if(e[a]==="\\")a+=1;else if(e[a]===n)return a+1;return e.length}function _r(e){let t="",n=0;for(;n<e.length;){const a=e.slice(n);let o;if(a.startsWith("//")){const s=e.indexOf(`
...
`)}function Ct(e){return e==="/"?"~":`~${e.replace(/\/$/,"")}`}function fa(e,t){return`<p class="ran"><span class="ps1">${R(e)} $</span> ${R(t)}</p>`}function fi(e,t){if(t.parent===null)return"";const n=e.childrenOf(t.route);if(n.length===0)return"";const a=n.map(o=>`<li><a class="entry" href="${o.route}"><code>${R(o.name)}/</code><span class="title">${R(o.title)}</span>`+(o.summary?`<span class="summary">${R(o.summary)}</span>`:"")+"</a></li>").join("");return`${fa(Ct(t.route),"ls")}
...
`)}}};function Mi(e){const t=e.filter(a=>a.startsWith("-")).flatMap(a=>a.slice(1).split("")),n=e.find(a=>!a.startsWith("-"))??".";return{flags:t,path:n}}function Ai(e,t,n){const a=n==="."?"":`${n.replace(/\/$/,"")}/`;return[{mode:"--r-",name:"README.md",title:e.title,href:e.route,run:`cat ${a}README.md`},...t.map(o=>({mode:"dr-x",name:`${o.name}/`,title:o.title,href:o.route}))]}function Cn(e){const t=e.run?` data-run="${R(e.run)}"`:"";return`<a href="${R(e.href)}"${t}>${R(e.name)}</a>`}function Ii(e,t){const n=(s,r)=>t?`${s.mode}  ${r.padEnd(20)}  ${s.title}`:r,a=s=>t?`${s.mode}  ${Cn(s)}${" ".repeat(Math.max(0,20-s.name.length))}  ${R(s.title)}`:Cn(s),o=t?[`total ${e.length}`]:[];return{text:[...o,...e.map(s=>n(s,s.name))].join(`
`),html:`<pre>${[...o,...e.map(a)].join(`
`)}</pre>`}}const Ei={name:"ls",usage:"ls [-l] [path]",description:"list what a directory holds; -l says what each is",run({site:e,cwd:t},n){const{flags:a,path:o}=Mi(n),s=a.find(h=>h!=="l");if(s)return{text:`ls: -${s}: no such option. Try ls -l`,error:!0};const r=qe(t,o),i=e.at(r);return i?Ii(Ai(i,e.childrenOf(r),o),a.includes("l")):{text:`ls: ${o}: no such directory`,error:!0}}},Ci={name:"pwd",usage:"pwd",description:"print where you are",run({cwd:e}){return{text:Ct(e)}}},wa=[Ei,$i,xi,Ci,Si,Ti];class Pi{context;constructor(t,n,a=wa){this.context={site:t,cwd:n,commands:a}}get prompt(){return`${Ct(this.context.cwd)} $`}moveTo(t){return this.context.site.at(t)?(this.context.cwd=t,!0):!1}run(t){const n=[];for(const[a="",...o]of ga(t)){const s=this.context.commands.find(i=>i.name===a),r=s?s.run(this.context,o):{text:`${a}: command not found. Try help`,error:!0};if(n.push(r),r.error)break}return n}complete(t){const n=t.split(/\s+/),a=n.pop()??"",o=n.length===0?"":`${n.join(" ")} `;return(n.length===0?this.commandNames():this.pathNames(a)).filter(r=>r.startsWith(a)).map(r=>o+r)}commandNames(){return this.context.commands.map(t=>t.name).sort()}pathNames(t){const n=t.lastIndexOf("/"),a=n<0?".":t.slice(0,n+1),o=qe(this.context.cwd,a);if(!this.context.site.at(o))return[];const s=n<0?"":a;return["README.md",...this.context.site.childrenOf(o).map(i=>`${i.name}/`)].map(i=>s+i)}}function ji(e,t,n){if(e==="")return"help";const o=[...[...t].reverse(),...n].find(s=>s.startsWith(e)&&s!==e);return o?o.slice(e.length):""}const Tt="shell-pending";function Oi(e){try{e&&sessionStorage.setItem(Tt,e)}catch{}}function Di(){try{const e=sessionStorage.getItem(Tt)??"";return sessionStorage.removeItem(Tt),e}catch{return""}}function Ri(){window.__stopTyped?.();const e=window.__typed??[];if(window.__typed=[],e.length===0)return null;const t=[];let n="";for(const a of e)a==="Enter"?(t.push(n),n=""):a==="Backspace"?n=n.slice(0,-1):n+=a;return{finished:t,unfinished:n}}function Ni(e,t,n={}){const a=document.querySelector(".terminal"),o=document.querySelector(".screen"),s=a?.querySelector("form.prompt"),r=s?.querySelector("input"),i=s?.querySelector(".line"),h=s?.querySelector(".suggest"),c=s?.querySelector(".ps1"),l=document.querySelector(".ran.end"),d=l?.querySelector(".ps1"),m=l?.querySelector(".line"),f=l?.querySelector(".typed");if(!a||!o||!s||!r||!i||!h||!c||!l||!d||!m||!f)return null;const u=()=>{c.textContent=g.prompt,d.textContent=g.prompt},g=new Pi(e,t,n.commands),v=new bi;let y=null;const k=b=>{o.append(b)},w=()=>{y?.remove(),y=null},S=()=>{const b=r.selectionStart??r.value.length;i.style.setProperty("--caret",String(b)),i.style.setProperty("--typed",String(r.value.length)),f.textContent=r.value,m.style.setProperty("--caret",String(b)),h.textContent=b===r.value.length?ji(r.value,v.lines,g.complete(r.value)):""},M=(b,T=b.length)=>{r.value=b,r.setSelectionRange(T,T),S()},A=b=>{if(b.clear&&(o.replaceChildren(),n.clearPage?.()),b.html){const T=p("div",{class:b.text?"listing-out":"cat"});T.innerHTML=b.html,k(T)}else b.text&&k(p("pre",{class:b.error?"error":""},b.text))},$=b=>{w();const T=p("p",{class:"echo"},p("span",{class:"ps1"},g.prompt),` ${b}`);k(T);let D=!1;const N=ga(b).map(C=>C.join(" "));for(let C=0;C<N.length;C+=1){const[L]=g.run(N[C]??"");if(L){if(A(L),L.html&&!L.text&&(D=!0),L.at&&!n.moveTo?.(L.at)){Oi(N.slice(C+1).join(" && ")),window.location.assign(L.at);return}if(L.error)break}}u(),S(),D?T.scrollIntoView({block:"start"}):window.scrollTo({top:document.documentElement.scrollHeight})},E=()=>{if(w(),r.value.trim()===""){M("help");return}const b=g.complete(r.value);b.length===1?M(b[0]??r.value):b.length>1&&(y=p("p",{class:"hint"},b.map(T=>T.split(" ").pop()).join("  ")),s.insertAdjacentElement("afterend",y),window.scrollTo({top:document.documentElement.scrollHeight}))};s.addEventListener("submit",b=>{b.preventDefault();const T=r.value.trim();M(""),T&&(v.add(T),$(T))});let P="";r.addEventListener("keydown",b=>{if(b.key==="Tab")b.preventDefault(),E();else if(b.key==="ArrowUp")b.preventDefault(),M(v.previous(r.value));else if(b.key==="ArrowDown")b.preventDefault(),M(v.next(r.value));else if(b.key==="ArrowRight"&&r.selectionStart===r.value.length&&h.textContent)b.preventDefault(),M(r.value+h.textContent);else if(b.ctrlKey&&!b.metaKey&&!b.altKey){const T=vi(b.key,r.value,r.selectionStart??r.value.length,P);if(!T)return;b.preventDefault(),w(),M(T.line,T.caret),P=T.killed}else w()});for(const b of["input","keyup","click","focus","select"])r.addEventListener(b,S);let O=!0;r.addEventListener("input",()=>{O&&r.value!==""&&window.scrollTo({top:document.documentElement.scrollHeight}),O=r.value===""}),document.addEventListener("selectionchange",()=>{document.activeElement===r&&S()}),o.addEventListener("click",b=>{const T=b.target?.closest("a[data-run]");T?.dataset.run&&(b.preventDefault(),$(T.dataset.run))}),window.addEventListener("keydown",b=>{const D=b.target?.matches("input, textarea, select, [contenteditable]")??!1,N=b.key.length===1&&!b.ctrlKey&&!b.metaKey&&!b.altKey;D||!N||r.focus({preventScroll:!1})}),s.addEventListener("click",()=>r.focus()),l.addEventListener("click",()=>r.focus()),S();const I=Di();I&&$(I);const x=Ri();if(x){for(const b of x.finished)b.trim()&&(v.add(b.trim()),$(b.trim()));M(x.unfinished),r.focus()}return{run:$,moveTo:b=>{g.moveTo(b)&&(o.replaceChildren(),u(),S())}}}const Li=[{file:"book/index.md",markdown:`---
