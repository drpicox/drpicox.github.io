...
`);return{columns:Math.max(...t.map(n=>n.length)),rows:t.length}}function $t(e,t){lt.get(e)?.();const n=t??{...kt,seed:Math.floor(Math.random()*16777215)},{columns:a,rows:o}=ur(e),s=Math.min(a,o*2),r=ca(n),i=new Et(s);e.dataset.seed=String(n.seed),e.title=`World ${n.seed}, ${r.mesh.faceCount.toLocaleString("en")} triangles`;const c=v=>{e.innerHTML=cr(ir(i.paint(r,{rotation:v}),s)),e.classList.add("grown")};if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return c(.6),$n(r,.6),lt.set(e,()=>{}),()=>{};let h=0,l=-1/0,d=-1/0;const m=performance.now(),g=la(e),u=v=>{const p=(v-m)/1e3/lr*Math.PI*2;g.onScreen()&&v-l>=hr&&(c(p),l=v),v-d>dr&&($n(r,p),d=v),h=requestAnimationFrame(u)};h=requestAnimationFrame(u);const w=()=>{cancelAnimationFrame(h),g.stop()};return lt.set(e,w),w}function mr(){const e=document.querySelector(".planet");return e?$t(e,ta()??void 0):()=>{}}const ne=360,pr=60,fr=1.4,xn=Math.PI*2/pr,Tn=Math.PI*4;function gr(e){const t=f("canvas",{class:"world",width:ne,height:ne}),n=t.getContext("2d");if(!n)return()=>{};const a={...kt,seed:Math.floor(Math.random()*16777215)},o=n.createImageData(ne,ne),s=new Et(ne,o.data),r=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let i,c=.6,h=-.38,l=!r,d=null,m=0,g=performance.now();const u=f("p",{class:"hint"}),w=document.querySelector(".planet"),v=(20*4**kt.levels).toLocaleString("en"),p=()=>{i=ca(a);const k=ta();u.textContent=`World ${a.seed}: ${i.mesh.faceCount.toLocaleString("en")} triangles. `+(k?`The header is keeping world ${k.seed}, ${(20*4**k.levels).toLocaleString("en")} triangles.`:`The header grows a new one every visit, ${v} triangles each.`),T.hidden=!k,S()},y=f("button",{type:"button",onclick:()=>{Os({...a}),w&&$t(w,{...a}),p()}},"Put it in the header"),T=f("button",{type:"button",hidden:!0,onclick:()=>{Ns(),w&&$t(w),p()}},"Let the header grow its own"),S=()=>{s.paint(i,{rotation:c,tilt:h}),n.putImageData(o,0,0)};let M=0;const I=la(t),$=k=>{const N=Math.min(.1,(k-g)/1e3);if(!d&&I.onScreen()){if(m!==0){m*=Math.exp(-N/fr);const b=l?xn:0;(Math.abs(m)<=b||Math.abs(m)<.01)&&(m=0)}m!==0?(c-=m*N,S()):l&&(c+=xn*N,S()),wt.send({byRadians:m*N,tiltedBy:0,seconds:N})}g=k,M=requestAnimationFrame($)};t.addEventListener("pointerdown",k=>{d={x:k.clientX,y:k.clientY,at:k.timeStamp},m=0,t.setPointerCapture(k.pointerId)}),t.addEventListener("pointermove",k=>{if(!d)return;const N=t.clientWidth||ne,b=(k.clientX-d.x)/N*Math.PI;c-=b;const x=h;h=Math.max(-1.2,Math.min(1.2,h-(k.clientY-d.y)/N*Math.PI)),wt.send({byRadians:b,tiltedBy:h-x,seconds:0});const j=Math.max(.004,(k.timeStamp-d.at)/1e3);m=Math.max(-Tn,Math.min(Tn,m*.4+b/j*.6)),d={x:k.clientX,y:k.clientY,at:k.timeStamp},S()}),t.addEventListener("pointerup",k=>{d&&k.timeStamp-d.at>120&&(m=0),d=null,g=performance.now()}),t.addEventListener("pointercancel",()=>{d=null,m=0});const C=f("input",{type:"number",min:0,value:a.seed,onchange:()=>{a.seed=Math.max(0,Math.floor(Number(C.value)||0)),p()}}),F=f("button",{type:"button",onclick:()=>{a.seed=Math.floor(Math.random()*16777215),C.value=String(a.seed),p()}},"Another world"),D=f("button",{type:"button",onclick:()=>{l=!l,D.textContent=l?"Hold still":"Turn"}},l?"Hold still":"Turn"),A=(k,N,b,x,j,O)=>{const E=f("output",{},O(a[k])),R=f("input",{type:"range",min:b,max:x,step:j,value:a[k],onchange:()=>{a[k]=Number(R.value),E.textContent=O(a[k]),p()},oninput:()=>{E.textContent=O(Number(R.value))}});return f("label",{},`${N}: `,E,R)};return e.append(t,f("div",{class:"row"},f("span",{},"Seed "),C,F,D,y,T),f("div",{class:"dials"},A("levels","Detail",2,6,1,k=>`${k} splits`),A("roughness","Roughness",.02,1,.01,k=>k.toFixed(2)),A("share","Sea",0,.98,.01,k=>`${Math.round(k*100)}%`)),u),p(),M=requestAnimationFrame($),()=>{cancelAnimationFrame(M),I.stop()}}const wr={name:"world",apps:{worlds:gr},install:()=>mr()},fe=[wr,as,Jo,zo,Za,ho,Ua,Ds,vo,Lo,To,rs];function Mn(e,t){const n=[];for(const a of document.querySelectorAll(".app[data-app]")){const o=e[a.dataset.app??""]?.(a,t);o&&n.push(o)}return()=>{for(const a of n)a()}}function yr(e){const t={},n=e.fields.theme;(n==="dark"||n==="light")&&(t["data-page-theme"]=n);const a=e.fields.sky;return a&&(t["data-sky"]=a),t}const br=["data-page-theme","data-sky"];function vr(e,t){return t==="/"?e==="/":e.startsWith(t)}const kr=7.8,Sn=17,$r=12,xr=8,ht=28,An=44,be=8,Tr=16;function Mr(e){const t=new Map;for(const p of e.nodes){const y=p.label.split(`
`),T=Math.max(...y.map(S=>S.length),1);t.set(p.id,{id:p.id,label:p.label,real:!0,rank:-1,along:Math.max(40,T*kr+$r*2),across:y.length*Sn+xr*2,pos:0,preds:[],succs:[]})}for(const p of e.edges){const y=t.get(p.from),T=t.get(p.to);if(!y||!T)throw new Error(`flow: edge ${p.from} --> ${p.to} names a node that is not there`);y.succs.push(T),T.preds.push(y)}Sr(t);const n=Ar(t,e),a=Ir(t);Er(a);const o=a.length,s=a.map(p=>Math.max(Sn,...p.map(y=>y.real?y.across:0))),r=[];let i=be;for(let p=0;p<o;p+=1)r.push(i),i+=(s[p]??0)+An;const c=p=>(r[p.rank]??0)+((s[p.rank]??0)-(p.real?p.across:0))/2,h=Math.max(...[...t.values()].map(p=>p.pos+p.along))+be,l=i-An+be,d=e.direction==="LR",m=(p,y)=>d?[y,p]:[p,y],g=[...t.values()].filter(p=>p.real).map(p=>{const[y,T]=m(p.pos,c(p));return{id:p.id,label:p.label,x:y,y:T,width:d?p.across:p.along,height:d?p.along:p.across}}),u=e.edges.map((p,y)=>{const T=n[y]??[],S=T[0],M=T[T.length-1];if(!S||!M)throw new Error("flow: an edge lost its ends");const I=[m(S.pos+S.along/2,c(S)+S.across),...T.slice(1,-1).map($=>m($.pos+$.along/2,c($)+(s[$.rank]??0)/2)),m(M.pos+M.along/2,c(M))];return p.label===void 0?{from:p.from,to:p.to,points:I}:{from:p.from,to:p.to,label:p.label,points:I}}),[w,v]=m(h,l);return{direction:e.direction,width:w,height:v,nodes:g,edges:u}}function Sr(e){const t=new Set,n=a=>{if(a.rank>=0)return a.rank;if(t.has(a))throw new Error(`flow: there is a cycle through ${a.id}, and a flow has a direction`);return t.add(a),a.rank=a.preds.length===0?0:Math.max(...a.preds.map(n))+1,t.delete(a),a.rank};for(const a of e.values())n(a)}function Ar(e,t){let n=0;return t.edges.map(a=>{const o=e.get(a.from),s=e.get(a.to);if(!o||!s)return[];const r=[o];let i=o;for(let c=o.rank+1;c<s.rank;c+=1){n+=1;const h={id:`\0${n}`,label:"",real:!1,rank:c,along:Tr,across:0,pos:0,preds:[i],succs:[]};e.set(h.id,h),i.succs.push(h),r.push(h),i=h}return i!==o&&(i.succs.push(s),s.preds.push(i),o.succs.splice(o.succs.indexOf(s),1),s.preds.splice(s.preds.indexOf(o),1)),r.push(s),r})}function Ir(e){const t=Math.max(...[...e.values()].map(r=>r.rank))+1,n=Array.from({length:t},()=>[]);for(const r of e.values())n[r.rank]?.push(r);const a=new Map,o=r=>r.forEach((i,c)=>a.set(i,c));n.forEach(o);const s=(r,i)=>i.length===0?a.get(r)??0:i.reduce((c,h)=>c+(a.get(h)??0),0)/i.length;for(let r=0;r<4;r+=1){for(let i=1;i<t;i+=1){const c=n[i]??[];c.sort((h,l)=>s(h,h.preds)-s(l,l.preds)),o(c)}for(let i=t-2;i>=0;i-=1){const c=n[i]??[];c.sort((h,l)=>s(h,h.succs)-s(l,l.succs)),o(c)}}return n}function Er(e){const t=r=>r.reduce((i,c)=>i+c.along,0)+ht*Math.max(0,r.length-1),n=Math.max(...e.map(t));for(const r of e){let i=be+(n-t(r))/2;for(const c of r)c.pos=i,i+=c.along+ht}const a=r=>r.pos+r.along/2,o=(r,i)=>{for(const c of r){const h=i(c);if(h.length===0)continue;const l=h.reduce((d,m)=>d+a(m),0)/h.length;c.pos=l-c.along/2}for(let c=1;c<r.length;c+=1){const h=r[c-1],l=r[c];if(!h||!l)continue;const d=h.pos+h.along+ht;l.pos<d&&(l.pos=d)}};for(let r=0;r<3;r+=1){for(let i=1;i<e.length;i+=1)o(e[i]??[],c=>c.preds);for(let i=e.length-2;i>=0;i-=1)o(e[i]??[],c=>c.succs)}const s=Math.min(...e.flat().map(r=>r.pos));for(const r of e.flat())r.pos+=be-s}const xt=/(\w[\w.-]*)(?:\[([^\]]*)\])?/,Cr=new RegExp(`^${xt.source}\\s*-->(?:\\|([^|]*)\\|)?\\s*${xt.source}$`),jr=new RegExp(`^${xt.source}$`),Pr=/^(?:flow\s+)?(TD|LR)$/i;function Dr(e){const t=new Map,n=[];let a="TD";const o=(i,c)=>{i&&(t.has(i)||t.set(i,i),c!==void 0&&t.set(i,c.replace(/\\n/g,`
`)))},s=e.split(`
`);let r=!0;return s.forEach((i,c)=>{const h=i.trim();if(h===""||h.startsWith("%"))return;if(r){r=!1;const m=Pr.exec(h);if(m){a=m[1]?.toUpperCase()==="LR"?"LR":"TD";return}}const l=Cr.exec(h);if(l){const[,m,g,u,w,v]=l;o(m,g),o(w,v),n.push(u===void 0?{from:m??"",to:w??""}:{from:m??"",to:w??"",label:u});return}const d=jr.exec(h);if(d){o(d[1],d[2]);return}throw new Error(`flow: cannot read line ${c+1}: "${h}"`)}),{direction:a,nodes:[...t].map(([i,c])=>({id:i,label:c})),edges:n}}const Or=20,In=17;function Nr(e){let t=5381;for(let n=0;n<e.length;n+=1)t=(t*33^e.charCodeAt(n))>>>0;return t.toString(36)}const B=e=>String(Math.round(e*10)/10);function Rr(e,t){const[n,...a]=e.points;if(!n)return"";let o=`M${B(n[0])},${B(n[1])}`,s=n;for(const r of a){const[i,c]=s,[h,l]=r,d=t?[(i+h)/2,c]:[i,(c+l)/2],m=t?[(i+h)/2,l]:[h,(c+l)/2];o+=` C${B(d[0])},${B(d[1])} ${B(m[0])},${B(m[1])} ${B(h)},${B(l)}`,s=r}return o}function Fr(e){const{points:t}=e,n=t[Math.floor((t.length-1)/2)]??[0,0],a=t[Math.ceil((t.length-1)/2)]??n;return[(n[0]+a[0])/2,(n[1]+a[1])/2]}function Lr(e){const t=Mr(Dr(e)),n=t.direction==="LR",a=`arrow-${Nr(e)}`,o=t.edges.map(c=>{const h=`<path class="edge" d="${Rr(c,n)}" marker-end="url(#${a})"/>`;if(c.label===void 0)return h;const[l,d]=Fr(c);return`${h}<text class="edge-label" x="${B(l)}" y="${B(d)}" text-anchor="middle" dominant-baseline="middle">${P(c.label)}</text>`}).join(""),s=t.nodes.map(c=>{const h=c.x+c.width/2,l=c.label.split(`
`),d=c.y+(c.height-l.length*In)/2,m=l.map((g,u)=>`<tspan x="${B(h)}" y="${B(d+Or-8+u*In)}">${P(g)}</tspan>`).join("");return`<g class="node"><rect x="${B(c.x)}" y="${B(c.y)}" width="${B(c.width)}" height="${B(c.height)}" rx="4"/><text text-anchor="middle" dominant-baseline="middle">${m}</text></g>`}).join(""),r=B(t.width),i=B(t.height);return`<figure class="flow"><svg class="flow" viewBox="0 0 ${r} ${i}" width="${r}" height="${i}" style="max-width: 100%; height: auto" role="img"><defs><marker id="${a}" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto-start-reverse"><path d="M0,0 L10,5 L0,10 z"/></marker></defs>${o}${s}</svg></figure>`}const Wr=new Set("var let const function return if else for while do break continue new this true false null undefined class extends import export from default async await throw try catch finally typeof instanceof in of switch case delete void yield".split(" "));function U(e,t){return`<span class="hl-${e}">${P(t)}</span>`}function ha(e,t,n){for(let a=t+1;a<e.length;a+=1)if(e[a]==="\\")a+=1;else if(e[a]===n)return a+1;return e.length}function Br(e){let t="",n=0;for(;n<e.length;){const a=e.slice(n);let o;if(a.startsWith("//")){const s=e.indexOf(`
`,n),r=s<0?e.length:s;t+=U("c",e.slice(n,r)),n=r}else if(a.startsWith("/*")){const s=e.indexOf("*/",n+2),r=s<0?e.length:s+2;t+=U("c",e.slice(n,r)),n=r}else if(a[0]==='"'||a[0]==="'"||a[0]==="`"){const s=ha(e,n,a[0]??"");t+=U("s",e.slice(n,s)),n=s}else if(o=/^[A-Za-z_$][\w$]*/.exec(a)){const s=o[0];t+=Wr.has(s)?U("k",s):P(s),n+=s.length}else(o=/^\d+(?:\.\d+)?/.exec(a))?(t+=U("n",o[0]),n+=o[0].length):(t+=P(a[0]??""),n+=1)}return t}function Hr(e){let t="",n=0;for(;n<e.length;){const a=e.slice(n);if(a.startsWith("<!--")){const s=e.indexOf("-->",n+4),r=s<0?e.length:s+3;t+=U("c",e.slice(n,r)),n=r;continue}const o=/^<(\/?)([A-Za-z][\w-]*)/.exec(a);if(!o){const s=e.indexOf("<",n+1),r=s<0?e.length:s;t+=P(e.slice(n,r)),n=r;continue}for(t+=`&lt;${o[1]}${U("t",o[2]??"")}`,n+=o[0].length;n<e.length&&e[n]!==">";){const s=e.slice(n);let r;if(r=/^\s+/.exec(s))t+=r[0],n+=r[0].length;else if(r=/^[A-Za-z_:][\w:.-]*/.exec(s))t+=U("a",r[0]),n+=r[0].length;else if(s[0]==="="&&(s[1]==='"'||s[1]==="'")){const i=ha(e,n+1,s[1]??"");t+=`=${U("s",e.slice(n+1,i))}`,n=i}else t+=P(s[0]??""),n+=1}e[n]===">"&&(t+="&gt;",n+=1)}return t}function _r(e,t){return t==="js"||t==="javascript"?Br(e):t==="html"?Hr(e):P(e)}function Gr(e,t){const a=/^https?:/.test(t)?' target="_blank" rel="noopener noreferrer"':"";return`<a href="${P(t)}"${a}>${e}</a>`}const Jr=["large","wide"];function Yr(e,t,n){const a=n&&Jr.includes(n)?` class="${n}"`:"";return`<img src="${P(t)}" alt="${P(e)}"${a}>`}const qr=/(`[^`]+`|!\[[^\]]*\]\([^)\s]+(?:\s+"[^"]*")?\)|\[[^\]]+\]\([^)\s]+\))/g,Ur=/^!\[([^\]]*)\]\(([^)\s]+)(?:\s+"([^"]*)")?\)$/,zr=/^\[([^\]]+)\]\(([^)\s]+)\)$/;function da(e){return e.split(qr).map(t=>{if(t.startsWith("`")&&t.endsWith("`")&&t.length>1)return`<code>${P(t.slice(1,-1))}</code>`;const n=Ur.exec(t);if(n)return Yr(n[1]??"",n[2]??"",n[3]);const a=zr.exec(t);return a?Gr(da(a[1]??""),a[2]??""):P(t)}).join("")}function Kr(e){const t=[];return e.replace(/<code>[\s\S]*?<\/code>/g,a=>`\0${t.push(a)-1}\0`).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*]+)\*/g,"$1<em>$2</em>").replace(/ {2,}\n/g,"<br>").replace(/\n/g," ").replace(/ -- /g," — ").replace(/\u0000(\d+)\u0000/g,(a,o)=>t[Number(o)]??"")}function z(e){return Kr(da(e))}function Vr(e){const t=e.split(`
`).map(g=>g.trim()).filter(Boolean),n=t.find(g=>!g.includes(" :: ")),a=t.filter(g=>g.includes(" :: ")).map(g=>{const u=g.indexOf(" :: ");return{left:g.slice(0,u).trim(),right:g.slice(u+4).trim()}}),o=a.filter(({left:g})=>g.startsWith("=")).map(({left:g,right:u})=>({value:Number(g.slice(1)),name:u})),s=a.filter(({left:g})=>!g.startsWith("=")).map(({left:g,right:u})=>{const[w="",v]=u.split("|").map(y=>y.trim()),p=Number(w.replace(/!$/,"").trim());return{label:g,value:p,shown:v??String(p),marked:w.endsWith("!")}}),r=Math.max(0,...s.map(({value:g})=>g),...o.map(({value:g})=>g))||1,i=g=>(Math.max(0,g)/r).toFixed(3),c=o[0],h=s.map(({label:g,value:u,shown:w,marked:v})=>`<tr${v?' class="marked"':""}><th scope="row">${z(g)}</th><td><span class="bar" style="--p:${i(u)}"></span><span class="value">${P(w)}</span></td></tr>`).join(""),l=c?` style="--rule:${i(c.value)}"`:"",d=c?` The line is ${P(c.name)}, at ${c.value}.`:"",m=n||c?`<figcaption>${n?z(n)+".":""}${d}</figcaption>`:"";return`<figure class="bars"><table${l}${c?' class="ruled"':""}><tbody>${h}</tbody></table>${m}</figure>`}const En=/^(?:[-*]|\d+\.)\s/;function Xr(e,t,n){if(!En.test(e[0]??""))return!1;const a=t.slice(n).find(o=>o.trim()!=="");return a!==void 0&&En.test(a)}function Zr(e){const t=[],n=e.replace(/\r\n?/g,`
`).split(`
...
`);return t==="flow"?Lr(n):t==="bars"?Vr(n):`<pre><code>${_r(n,t)}</code></pre>`}function ni(e,t){const n=[];for(const a of e)t.test(a)?n.push(a.replace(t,"")):n.length&&(n[n.length-1]+=`
...
`)}function Ct(e){return e==="/"?"~":`~${e.replace(/\/$/,"")}`}function ma(e,t){return`<p class="ran"><span class="ps1">${P(e)} $</span> ${P(t)}</p>`}function ui(e,t){if(t.parent===null)return"";const n=e.childrenOf(t.route);if(n.length===0)return"";const a=n.map(o=>`<li><a class="entry" href="${o.route}"><code>${P(o.name)}/</code><span class="title">${P(o.title)}</span>`+(o.summary?`<span class="summary">${P(o.summary)}</span>`:"")+"</a></li>").join("");return`${ma(Ct(t.route),"ls")}
...
`)}}};function xi(e){const t=e.filter(a=>a.startsWith("-")).flatMap(a=>a.slice(1).split("")),n=e.find(a=>!a.startsWith("-"))??".";return{flags:t,path:n}}function Ti(e,t,n){const a=n==="."?"":`${n.replace(/\/$/,"")}/`;return[{mode:"--r-",name:"README.md",title:e.title,href:e.route,run:`cat ${a}README.md`},...t.map(o=>({mode:"dr-x",name:`${o.name}/`,title:o.title,href:o.route}))]}function Cn(e){const t=e.run?` data-run="${P(e.run)}"`:"";return`<a href="${P(e.href)}"${t}>${P(e.name)}</a>`}function Mi(e,t){const n=(s,r)=>t?`${s.mode}  ${r.padEnd(20)}  ${s.title}`:r,a=s=>t?`${s.mode}  ${Cn(s)}${" ".repeat(Math.max(0,20-s.name.length))}  ${P(s.title)}`:Cn(s),o=t?[`total ${e.length}`]:[];return{text:[...o,...e.map(s=>n(s,s.name))].join(`
`),html:`<pre>${[...o,...e.map(a)].join(`
`)}</pre>`}}const Si={name:"ls",usage:"ls [-l] [path]",description:"list what a directory holds; -l says what each is",run({site:e,cwd:t},n){const{flags:a,path:o}=xi(n),s=a.find(c=>c!=="l");if(s)return{text:`ls: -${s}: no such option. Try ls -l`,error:!0};const r=Ge(t,o),i=e.at(r);return i?Mi(Ti(i,e.childrenOf(r),o),a.includes("l")):{text:`ls: ${o}: no such directory`,error:!0}}},Ai={name:"pwd",usage:"pwd",description:"print where you are",run({cwd:e}){return{text:Ct(e)}}},fa=[Si,vi,bi,Ai,$i,ki];class Ii{context;constructor(t,n,a=fa){this.context={site:t,cwd:n,commands:a}}get prompt(){return`${Ct(this.context.cwd)} $`}moveTo(t){return this.context.site.at(t)?(this.context.cwd=t,!0):!1}run(t){const n=[];for(const[a="",...o]of pa(t)){const s=this.context.commands.find(i=>i.name===a),r=s?s.run(this.context,o):{text:`${a}: command not found. Try help`,error:!0};if(n.push(r),r.error)break}return n}complete(t){const n=t.split(/\s+/),a=n.pop()??"",o=n.length===0?"":`${n.join(" ")} `;return(n.length===0?this.commandNames():this.pathNames(a)).filter(r=>r.startsWith(a)).map(r=>o+r)}commandNames(){return this.context.commands.map(t=>t.name).sort()}pathNames(t){const n=t.lastIndexOf("/"),a=n<0?".":t.slice(0,n+1),o=Ge(this.context.cwd,a);if(!this.context.site.at(o))return[];const s=n<0?"":a;return["README.md",...this.context.site.childrenOf(o).map(i=>`${i.name}/`)].map(i=>s+i)}}function Ei(e,t,n){if(e==="")return"help";const o=[...[...t].reverse(),...n].find(s=>s.startsWith(e)&&s!==e);return o?o.slice(e.length):""}const Tt="shell-pending";function Ci(e){try{e&&sessionStorage.setItem(Tt,e)}catch{}}function ji(){try{const e=sessionStorage.getItem(Tt)??"";return sessionStorage.removeItem(Tt),e}catch{return""}}function Pi(){window.__stopTyped?.();const e=window.__typed??[];if(window.__typed=[],e.length===0)return null;const t=[];let n="";for(const a of e)a==="Enter"?(t.push(n),n=""):a==="Backspace"?n=n.slice(0,-1):n+=a;return{finished:t,unfinished:n}}function Di(e,t,n={}){const a=document.querySelector(".terminal"),o=document.querySelector(".screen"),s=a?.querySelector("form.prompt"),r=s?.querySelector("input"),i=s?.querySelector(".line"),c=s?.querySelector(".suggest"),h=s?.querySelector(".ps1"),l=document.querySelector(".ran.end"),d=l?.querySelector(".ps1"),m=l?.querySelector(".line"),g=l?.querySelector(".typed");if(!a||!o||!s||!r||!i||!c||!h||!l||!d||!m||!g)return null;const u=()=>{h.textContent=w.prompt,d.textContent=w.prompt},w=new Ii(e,t,n.commands),v=new gi;let p=null;const y=b=>{o.append(b)},T=()=>{p?.remove(),p=null},S=()=>{const b=r.selectionStart??r.value.length;i.style.setProperty("--caret",String(b)),i.style.setProperty("--typed",String(r.value.length)),g.textContent=r.value,m.style.setProperty("--caret",String(b)),c.textContent=b===r.value.length?Ei(r.value,v.lines,w.complete(r.value)):""},M=(b,x=b.length)=>{r.value=b,r.setSelectionRange(x,x),S()},I=b=>{if(b.clear&&(o.replaceChildren(),n.clearPage?.()),b.html){const x=f("div",{class:b.text?"listing-out":"cat"});x.innerHTML=b.html,y(x)}else b.text&&y(f("pre",{class:b.error?"error":""},b.text))},$=b=>{T();const x=f("p",{class:"echo"},f("span",{class:"ps1"},w.prompt),` ${b}`);y(x);let j=!1;const O=pa(b).map(E=>E.join(" "));for(let E=0;E<O.length;E+=1){const[R]=w.run(O[E]??"");if(R){if(I(R),R.html&&!R.text&&(j=!0),R.at&&!n.moveTo?.(R.at)){Ci(O.slice(E+1).join(" && ")),window.location.assign(R.at);return}if(R.error)break}}u(),S(),j?x.scrollIntoView({block:"start"}):window.scrollTo({top:document.documentElement.scrollHeight})},C=()=>{if(T(),r.value.trim()===""){M("help");return}const b=w.complete(r.value);b.length===1?M(b[0]??r.value):b.length>1&&(p=f("p",{class:"hint"},b.map(x=>x.split(" ").pop()).join("  ")),s.insertAdjacentElement("afterend",p),window.scrollTo({top:document.documentElement.scrollHeight}))};s.addEventListener("submit",b=>{b.preventDefault();const x=r.value.trim();M(""),x&&(v.add(x),$(x))});let F="";r.addEventListener("keydown",b=>{if(b.key==="Tab")b.preventDefault(),C();else if(b.key==="ArrowUp")b.preventDefault(),M(v.previous(r.value));else if(b.key==="ArrowDown")b.preventDefault(),M(v.next(r.value));else if(b.key==="ArrowRight"&&r.selectionStart===r.value.length&&c.textContent)b.preventDefault(),M(r.value+c.textContent);else if(b.ctrlKey&&!b.metaKey&&!b.altKey){const x=wi(b.key,r.value,r.selectionStart??r.value.length,F);if(!x)return;b.preventDefault(),T(),M(x.line,x.caret),F=x.killed}else T()});for(const b of["input","keyup","click","focus","select"])r.addEventListener(b,S);let D=!0;r.addEventListener("input",()=>{D&&r.value!==""&&window.scrollTo({top:document.documentElement.scrollHeight}),D=r.value===""}),document.addEventListener("selectionchange",()=>{document.activeElement===r&&S()}),o.addEventListener("click",b=>{const x=b.target?.closest("a[data-run]");x?.dataset.run&&(b.preventDefault(),$(x.dataset.run))}),window.addEventListener("keydown",b=>{const j=b.target?.matches("input, textarea, select, [contenteditable]")??!1,O=b.key.length===1&&!b.ctrlKey&&!b.metaKey&&!b.altKey;j||!O||r.focus({preventScroll:!1})}),s.addEventListener("click",()=>r.focus()),l.addEventListener("click",()=>r.focus()),S();const A=ji();A&&$(A);const k=Pi();if(k){for(const b of k.finished)b.trim()&&(v.add(b.trim()),$(b.trim()));M(k.unfinished),r.focus()}return{run:$,moveTo:b=>{w.moveTo(b)&&(o.replaceChildren(),u(),S())}}}const Oi=[{file:"book/index.md",markdown:`---
