const V=e=>Math.max(0,Math.min(100,e));function Me(e){const{focus:t,fatigue:n,featureSize:o,weeks:a,calendar:s,meetingTypes:r}=e,i=[];let l=0,c=0;for(let d=0;d<a;d+=1)for(let u=0;u<5;u+=1){let p=0,g=0;for(let f=0;f<8;f+=1){const y=r[s[`${u}-${f}`]??""];if(y){p=V(p+y.focus),g=V(g+y.fatigue),i.push({week:d,day:u,hour:f,inMeeting:!0,hourFocus:p,hourFatigue:g,hourProductivity:0,accumulatedProductivity:l,completedFeatures:c,featureCompleted:!1});continue}p=V(p+t),g=V(g+n);const w=V(p-g),b=o-l,T=w>b,C=T?b:w;T?(c+=1,l=0):l+=C,i.push({week:d,day:u,hour:f,inMeeting:!1,hourFocus:p,hourFatigue:g,hourProductivity:C,accumulatedProductivity:l,completedFeatures:c,featureCompleted:T}),T&&(p=0)}}return i}function ne(){return Array.from({length:8},()=>new Array(5).fill(0))}function oe(e,{hour:t,day:n},o){const a=e[t];a&&(a[n]=(a[n]??0)+o)}function Ee(e,{featureSize:t,weeks:n}){const o=e[e.length-1],a=o?.completedFeatures??0,s=o?.accumulatedProductivity??0,r=a+Math.round(10*s/t)/10,i=a*t+s,l=Array.from({length:5},()=>({productivity:0,features:0,meetings:0})),c={focus:ne(),fatigue:ne(),productivity:ne(),features:ne()};for(const u of e){const p=l[u.day];p.productivity+=u.hourProductivity,u.featureCompleted&&(p.features+=1),u.inMeeting&&(p.meetings+=1),oe(c.focus,u,u.hourFocus),oe(c.fatigue,u,u.hourFatigue),oe(c.productivity,u,u.hourProductivity),u.featureCompleted&&oe(c.features,u,1)}const d=u=>u.map(p=>p.map(g=>n>0?g/n:0));return{totalFeatures:r,totalProductivity:i,averageFeaturesPerWeek:n>0?r/n:0,averageProductivityPerWeek:n>0?i/n:0,days:l,hours:{focus:d(c.focus),fatigue:d(c.fatigue),productivity:d(c.productivity),features:c.features}}}const se=480,K=240,E={top:10,right:10,bottom:34,left:36};function Ke(e,t,n,o){const a=se-E.left-E.right,s=K-E.top-E.bottom,r=c=>E.top+s-(e>0?c/e*s:0),i=o.map(c=>`<line class="grid" x1="${E.left}" x2="${se-E.right}" y1="${r(c)}" y2="${r(c)}"/><text x="${E.left-4}" y="${r(c)+3}" text-anchor="end">${c}</text>`).join(""),l=(n>1?[1,Math.ceil(n/2),n]:[]).filter((c,d,u)=>u.indexOf(c)===d).map(c=>`<text x="${E.left+(c-1)/Math.max(1,n-1)*a}" y="${K-E.bottom+14}" text-anchor="middle">${c}</text>`).join("");return`${i}${l}<text x="${E.left+a/2}" y="${K-6}" text-anchor="middle">${t.x}</text><text transform="translate(9 ${E.top+s/2}) rotate(-90)" text-anchor="middle">${t.y}</text>`}function qe(e){if(e<=0)return[0];const t=10**Math.floor(Math.log10(e)),n=e/t>=5?t:e/t>=2?t/2:t/5,o=[];for(let a=0;a<=e;a+=n)o.push(Math.round(a*100)/100);return o}function vt(e,t){const n=Math.max(...e.map(d=>d.values.length),1),o=Math.max(1,...e.flatMap(d=>d.values)),a=se-E.left-E.right,s=K-E.top-E.bottom,r=d=>E.left+d/Math.max(1,n-1)*a,i=d=>E.top+s-d/o*s,l=e.map(d=>{const u=d.values.map((p,g)=>`${r(g).toFixed(1)},${i(p).toFixed(1)}`).join(" ");return`<polyline class="line ${d.className}" points="${u}"><title>${d.name}</title></polyline>`}).join(""),c=e.map((d,u)=>`<rect class="${d.className}" x="${E.left+u*90}" y="${K-E.bottom+20}" width="10" height="3"/><text x="${E.left+u*90+14}" y="${K-E.bottom+24}">${d.name}</text>`).join("");return`<svg viewBox="0 0 ${se} ${K}" role="img" aria-label="${t.y} by ${t.x}">${Ke(o,t,n,qe(o))}${l}${c}</svg>`}const Ce=480,Z=240,F={top:10,right:10,bottom:34,left:36};function Ge(e,t,n){const o=Math.max(...e.map(f=>f.values.length),1),a=Math.max(1,...e.flatMap(f=>f.values)),s=Ce-F.left-F.right,r=Z-F.top-F.bottom,i=s/o,l=i*.7/e.length,c=f=>F.top+r-f/a*r,d=e.map((f,y)=>f.values.map((w,b)=>{const T=F.left+b*i+i*.15+y*l;return`<rect class="${f.className}" x="${T.toFixed(1)}" y="${c(w).toFixed(1)}" width="${l.toFixed(1)}" height="${(F.top+r-c(w)).toFixed(1)}"><title>${f.name}: ${Math.round(w*10)/10}</title></rect>`}).join("")).join(""),u=(n??[]).map((f,y)=>`<text x="${F.left+y*i+i/2}" y="${Z-F.bottom+14}" text-anchor="middle">${f}</text>`).join(""),p=e.map((f,y)=>`<rect class="${f.className}" x="${F.left+y*90}" y="${Z-F.bottom+20}" width="10" height="3"/><text x="${F.left+y*90+14}" y="${Z-F.bottom+24}">${f.name}</text>`).join(""),g=Ke(a,t,n?0:o,qe(a));return`<svg viewBox="0 0 ${Ce} ${Z}" role="img" aria-label="${t.y} by ${t.x}">${g}${d}${u}${p}</svg>`}function h(e,t={},...n){const o=document.createElement(e);for(const[a,s]of Object.entries(t))s===void 0||s===!1||(typeof s=="function"?o.addEventListener(a.slice(2).toLowerCase(),s):s===!0?o.setAttribute(a,""):o.setAttribute(a,String(s)));for(const a of n)a==null||a===!1||o.append(a);return o}const fe=["Mon","Tue","Wed","Thu","Fri"],Ue=Array.from({length:8},(e,t)=>`${9+t}:00`);function bt(e){return e<=500?e:e<=750?500+(e-500)*2:e<1e3?1e3+(e-750)*35:1e4}function xt(e){return e<=500?e:e<=1e3?500+(e-500)/2:e<1e4?750+(e-1e3)/35:1e3}function ae(e,t){const n=t.flat(),o=Math.min(...n),a=Math.max(...n),s=h("div",{class:"week"},h("span"),...fe.map(r=>h("span",{class:"head"},r)));return t.forEach((r,i)=>{s.append(h("span",{class:"hour"},Ue[i]??""));for(const l of r){const c=a>o?(l-o)/(a-o):0;s.append(h("span",{class:"cell",style:`--heat:${(.1+c*.9).toFixed(2)}`},String(Math.round(l))))}}),h("div",{},h("h4",{},e),s)}function kt(e){const t={focus:25,fatigue:15,featureSize:300,weeks:8},n={"🍽️ Lunch":{focus:-100,fatigue:-100},"🏃 Sprint plan":{focus:-100,fatigue:50},"😴 Boring":{focus:-50,fatigue:-25}},o={};for(let k=0;k<5;k+=1)o[`${k}-3`]="🍽️ Lunch";let a="🏃 Sprint plan",s=null;const r=h("div",{class:"figures"}),i=h("div",{class:"chart"}),l=h("div",{class:"maps"}),c=h("div",{class:"week"}),d=h("select"),u=h("input",{type:"number",min:-100,max:100}),p=h("input",{type:"number",min:-100,max:100}),g=h("input",{type:"text",placeholder:"New meeting name",size:16}),f=(k,S,L,I,P=A=>A,v=A=>A)=>{const A=h("output",{},String(t[k])),D=h("input",{type:"range",min:L,max:I,value:v(t[k]),oninput:()=>{t[k]=P(Number(D.value)),A.textContent=String(t[k]),O()}});return h("label",{},`${S}: `,A,D)},y=h("div",{class:"dials"},f("focus","Focus an hour",0,100),f("fatigue","Fatigue an hour",0,100),f("featureSize","Feature size",0,1e3,bt,xt),f("weeks","Weeks",1,16));function w(){d.replaceChildren(...Object.keys(n).map(S=>h("option",{value:S,selected:S===a},S)));const k=n[a];u.value=String(k?.focus??0),p.value=String(k?.fatigue??0)}d.addEventListener("change",()=>{a=d.value,w()});const b=()=>{n[a]={focus:Number(u.value)||0,fatigue:Number(p.value)||0},O()};u.addEventListener("change",b),p.addEventListener("change",b);const T=()=>{const k=g.value.trim();!k||n[k]||(n[k]={focus:0,fatigue:0},a=k,g.value="",w())},C=h("div",{class:"row"},h("span",{},"Paint: "),d,h("span",{},"focus "),u,h("span",{},"fatigue "),p,g,h("button",{type:"button",onclick:T},"Add"));let m=null;const x=k=>{if(m==="add"&&!o[k])o[k]=a;else if(m==="remove"&&o[k])delete o[k];else return;O()};function $(){c.replaceChildren(h("span"),...fe.map(k=>h("span",{class:"head"},k))),Ue.forEach((k,S)=>{c.append(h("span",{class:"hour"},k));for(let L=0;L<5;L+=1){const I=`${L}-${S}`,P=o[I];c.append(h("span",{class:P?"slot meeting":"slot",title:P??"free",onpointerdown:v=>{v.preventDefault(),m=o[I]?"remove":"add",x(I)},onpointerenter:()=>{m&&x(I)}},P?P.slice(0,2):""))}})}window.addEventListener("pointerup",()=>{m=null});const M=h("div",{class:"row"}),R=()=>{s={summary:Ee(Me({...t,calendar:o,meetingTypes:n}),t),weeks:t.weeks},O()},W=()=>{s=null,O()};function O(){$();const k=Me({...t,calendar:o,meetingTypes:n}),S=Ee(k,t),L=t.weeks*5*8;r.replaceChildren(h("div",{class:"clean"},h("strong",{},S.totalFeatures.toFixed(1)),"features finished"),h("div",{},h("strong",{},S.averageFeaturesPerWeek.toFixed(2)),"features a week"),h("div",{},h("strong",{},Math.round(S.totalProductivity/L).toString()),"productivity an hour"),h("div",{},h("strong",{},String(L)),"hours simulated")),M.replaceChildren(s?h("span",{},`Baseline: ${s.summary.averageFeaturesPerWeek.toFixed(2)} features a week over ${s.weeks} weeks; now ${S.averageFeaturesPerWeek.toFixed(2)}. `):h("span",{},"Keep this run to compare against: "),h("button",{type:"button",onclick:R},s?"Save again":"Save as baseline")),s&&M.append(h("button",{type:"button",onclick:W},"Clear")),i.innerHTML=Ge([{name:"Productivity",className:"clean",values:S.days.map(I=>I.productivity/t.weeks)},{name:"Features ×100",className:"debt",values:S.days.map(I=>I.features/t.weeks*100)}],{x:"",y:"A day, on average"},fe),i.prepend(h("h4",{},"The shape of a week")),l.replaceChildren(ae("Focus",S.hours.focus),ae("Fatigue",S.hours.fatigue),ae("Productivity",S.hours.productivity),ae("Features finished",S.hours.features))}w(),e.append(y,C,h("div",{class:"charts"},c,i),r,M,l),O()}const ue=20;function $t(e){const{baseTime:t,shortcutFactor:n,interestRate:o,timeHorizon:a}=e,s=[];let r=null;const i=t;let l=t*(1-n),c=0,d=0,u=0,p=0,g=0,f=0;for(let y=0;y<a*ue;){for(;g<=y;)c+=1,u+=1,g+=i;for(;f<=y;)d+=1,p+=1,f+=l,l*=1+o;if(y+=1,y%ue===0){const w=y/ue;s.push({month:w,cleanCumulative:c,debtCumulative:d,cleanMonthly:u,debtMonthly:p,debtFeatureCost:l}),u=0,p=0,r===null&&c>d&&(r=w)}}return{months:s,breakEvenMonth:r}}const Tt=[{key:"baseTime",label:"Base time",min:1,max:30,step:1,show:e=>`${e} days`},{key:"shortcutFactor",label:"Shortcuts",min:0,max:.9,step:.05,show:e=>`${Math.round(e*100)}%`},{key:"interestRate",label:"Interest",min:0,max:1,step:.01,show:e=>`${Math.round(e*100)}%`},{key:"timeHorizon",label:"Timeline",min:6,max:60,step:1,show:e=>`${e} months`}];function St(e){const t={baseTime:20,shortcutFactor:.25,interestRate:.1,timeHorizon:24},n=h("div",{class:"figures"}),o=h("div",{class:"chart"}),a=h("div",{class:"chart"}),s=h("p"),r=h("div",{class:"dials"},...Tt.map(l=>{const c=h("output",{},l.show(t[l.key])),d=h("input",{type:"range",min:l.min,max:l.max,step:l.step,value:t[l.key],oninput:()=>{t[l.key]=Number(d.value),c.textContent=l.show(t[l.key]),i()}});return h("label",{},`${l.label}: `,c,d)}));function i(){const{months:l,breakEvenMonth:c}=$t(t),d=l[l.length-1],u=d?.cleanCumulative??0,p=d?.debtCumulative??0,g=u>0?(u-p)/u*100:0,f=Math.abs(g)<.1?"Even":g>0?"Loss":"Gain",y=Math.abs(g)<.1?"≈0%":`${Math.abs(g).toFixed(1)}%`;n.replaceChildren(h("div",{class:"clean"},h("strong",{},String(u)),"clean features"),h("div",{class:"debt"},h("strong",{},String(p)),"debt features"),h("div",{},h("strong",{},c?`month ${c}`:"never"),"break-even"),h("div",{},h("strong",{},y),`${f.toLowerCase()} on the shortcut road`)),o.innerHTML=vt([{name:"Clean",className:"clean",values:l.map(w=>w.cleanCumulative)},{name:"Debt-driven",className:"debt",values:l.map(w=>w.debtCumulative)}],{x:"Months",y:"Features"}),o.prepend(h("h4",{},"Cumulative features")),a.innerHTML=Ge([{name:"Clean",className:"clean",values:l.slice(1).map(w=>w.cleanMonthly)},{name:"Debt-driven",className:"debt",values:l.slice(1).map(w=>w.debtMonthly)}],{x:"Months",y:"Features a month"}),a.prepend(h("h4",{},"Monthly delivery rate")),s.textContent=t.interestRate===0?"With no interest there is no compound slowdown, and the shortcut simply wins. That is the one case that does not happen to real code.":c?`${Math.round(t.shortcutFactor*100)}% saved at first, ${Math.round(t.interestRate*100)}% interest on every feature: clean development overtakes at month ${c}, and by month ${t.timeHorizon} the shortcut road has delivered ${y} less.`:`${Math.round(t.shortcutFactor*100)}% saved at first, ${Math.round(t.interestRate*100)}% interest on every feature: in ${t.timeHorizon} months the clean road has not yet caught up. Give it longer, or raise the interest.`}e.append(r,n,h("div",{class:"charts"},o,a),s),i()}const _=(1+Math.sqrt(5))/2,Mt=[[-1,_,0],[1,_,0],[-1,-_,0],[1,-_,0],[0,-1,_],[0,1,_],[0,-1,-_],[0,1,-_],[_,0,-1],[_,0,1],[-_,0,-1],[-_,0,1]],Et=[[0,11,5],[0,5,1],[0,1,7],[0,7,10],[0,10,11],[1,5,9],[5,11,4],[11,10,2],[10,7,6],[7,1,8],[3,9,4],[3,4,2],[3,2,6],[3,6,8],[3,8,9],[4,9,5],[2,4,11],[6,2,10],[8,6,7],[9,8,1]];function Ct(){const e=Mt.map(([t,n,o])=>{const a=Math.hypot(t,n,o);return{direction:[t/a,n/a,o/a],radius:1}});return ze(e,Et.map(t=>[...t]))}function At(e,t){const n=Array.from({length:e.vertexCount},(r,i)=>({direction:[e.directions[i*3]??0,e.directions[i*3+1]??0,e.directions[i*3+2]??0],radius:e.radii[i]??1})),o=new Map,a=(r,i)=>{const l=r<i?`${r}:${i}`:`${i}:${r}`,c=o.get(l);if(c!==void 0)return c;const d=n[r],u=n[i],[p,g,f]=d.direction,[y,w,b]=u.direction,T=Math.hypot(p*d.radius-y*u.radius,g*d.radius-w*u.radius,f*d.radius-b*u.radius),[C,m,x]=[(p+y)/2,(g+w)/2,(f+b)/2],$=Math.hypot(C,m,x)||1;n.push({direction:[C/$,m/$,x/$],radius:(d.radius+u.radius)/2+t(T)});const M=n.length-1;return o.set(l,M),M},s=[];for(let r=0;r<e.faceCount;r+=1){const i=e.faces[r*3],l=e.faces[r*3+1],c=e.faces[r*3+2],d=a(i,l),u=a(l,c),p=a(c,i);s.push([i,d,p],[l,u,d],[c,p,u],[d,u,p])}return ze(n,s)}function ze(e,t){const n=new Float32Array(e.length*3),o=new Float32Array(e.length);e.forEach(({direction:s,radius:r},i)=>{n[i*3]=s[0],n[i*3+1]=s[1],n[i*3+2]=s[2],o[i]=r});const a=new Uint32Array(t.length*3);return t.forEach(([s,r,i],l)=>{a[l*3]=s,a[l*3+1]=r,a[l*3+2]=i}),{directions:n,radii:o,faces:a,faceCount:t.length,vertexCount:e.length}}function It(e,t){const n=e.radii[t]??1;return[(e.directions[t*3]??0)*n,(e.directions[t*3+1]??0)*n,(e.directions[t*3+2]??0)*n]}function Dt(e){let t=e>>>0;return()=>{t=t+1831565813>>>0;let n=Math.imul(t^t>>>15,1|t);return n=n+Math.imul(n^n>>>7,61|n)^n,((n^n>>>14)>>>0)/4294967296}}function Rt(e){const t=Ct();return{seed:e,mesh:t,temperature:new Float32Array(t.vertexCount),faceColour:new Uint8ClampedArray(t.faceCount*3),seaRadius:0}}function Pt(e,t){return{...e,mesh:t,temperature:new Float32Array(t.vertexCount),faceColour:new Uint8ClampedArray(t.faceCount*3)}}function Ft(e,t){return Math.abs(e.mesh.directions[t*3+1]??0)}function Je(e,t,n){const o=e.mesh.faces[n*3]??0,a=e.mesh.faces[n*3+1]??0,s=e.mesh.faces[n*3+2]??0;return((t[o]??0)+(t[a]??0)+(t[s]??0))/3}function Wt(e,t){return Je(e,e.mesh.radii,t)}const Xe=(e=4,t=.28)=>n=>{const o=Dt(n.seed);let a=n.mesh;for(let s=0;s<e;s+=1)a=At(a,r=>r*t*(o()-.5));return Pt(n,a)},Ve=({equator:e=1,pole:t=.05,peak:n=.42}={})=>o=>{const a=new Float32Array(o.mesh.vertexCount),s=o.mesh.radii,r=s.reduce((c,d)=>Math.min(c,d),1/0),l=s.reduce((c,d)=>Math.max(c,d),-1/0)-r||1;for(let c=0;c<o.mesh.vertexCount;c+=1){const d=((s[c]??1)-r)/l,u=Ft(o,c)**2.2;a[c]=e+(t-e)*u+(n-e)*d}return{...o,temperature:a}},Ze=(e=.55)=>t=>{const n=Float32Array.from(t.mesh.radii).sort(),o=Math.min(n.length-1,Math.floor(n.length*e)),a=n[o]??1,s=Float32Array.from(t.mesh.radii,r=>Math.max(r,a));return{...t,mesh:{...t.mesh,radii:s},seaRadius:a}},Ot=[24,92,168],Lt=[62,176,206],Nt=[214,196,138],Ae=[190,158,84],he=[70,138,66],_t=[74,104,76],Ht=[136,128,116],Ie=[238,243,247];function B(e,t,n){const o=Math.min(1,Math.max(0,n));return[e[0]+(t[0]-e[0])*o,e[1]+(t[1]-e[1])*o,e[2]+(t[2]-e[2])*o]}function Yt(e){return e>.78?Ae:e>.62?B(he,Ae,(e-.62)/.16):e>.3?he:B(_t,he,(e-.12)*5.5)}const Qe=e=>{const t=new Uint8ClampedArray(e.mesh.faceCount*3),n=e.mesh.radii.reduce((a,s)=>Math.max(a,s),-1/0),o=Math.max(1e-6,n-e.seaRadius);for(let a=0;a<e.mesh.faceCount;a+=1){const s=(Wt(e,a)-e.seaRadius)/o,r=Je(e,e.temperature,a);let i;s<=.002?(i=B(Lt,Ot,.55),r<.16&&(i=B(i,Ie,(.16-r)*6))):(i=B(Nt,Yt(r),Math.min(1,s*9)),i=B(i,Ht,Math.max(0,s-.55)*2.2),r<.26&&(i=B(i,Ie,(.26-r)*4))),t[a*3]=i[0],t[a*3+1]=i[1],t[a*3+2]=i[2]}return{...e,faceColour:t}},jt=[Xe(),Ve(),Ze(),Qe];function Bt(e,t=jt){return t.reduce((n,o)=>o(n),Rt(e))}function et(e){return Bt(e.seed,[Xe(e.levels,e.roughness),Ve(),Ze(e.share),Qe])}const De=.3,Kt=[-.5,.45,.74],qt=1.02;function Re([e,t,n]){const o=Math.hypot(e,t,n)||1;return[e/o,t/o,n/o]}function ve(e,t,n){const o=new Uint8ClampedArray(t*t*4),a=new Float32Array(t*t).fill(-1/0),[s,r,i]=Re(n.light??Kt),l=n.tilt??-.38,c=Math.cos(l),d=Math.sin(l),u=Math.cos(n.rotation),p=Math.sin(n.rotation),g=e.mesh.radii.reduce((b,T)=>Math.max(b,T),1),f=t/(2*g*qt),y=new Float32Array(e.mesh.vertexCount*3),w=new Float32Array(e.mesh.vertexCount*3);for(let b=0;b<e.mesh.vertexCount;b+=1){const[T,C,m]=It(e.mesh,b),x=T*u-m*p,$=T*p+m*u,M=C*c+$*d,R=-C*d+$*c;y[b*3]=x,y[b*3+1]=M,y[b*3+2]=R,w[b*3]=t/2+x*f,w[b*3+1]=t/2-M*f,w[b*3+2]=R}for(let b=0;b<e.mesh.faceCount;b+=1){const T=e.mesh.faces[b*3]??0,C=e.mesh.faces[b*3+1]??0,m=e.mesh.faces[b*3+2]??0,x=w[T*3],$=w[T*3+1],M=w[T*3+2],R=w[C*3],W=w[C*3+1],O=w[C*3+2],k=w[m*3],S=w[m*3+1],L=w[m*3+2],I=(R-x)*(S-$)-(W-$)*(k-x);if(I>=0)continue;const P=y[T*3],v=y[T*3+1],A=y[T*3+2],D=y[C*3]-P,N=y[C*3+1]-v,Y=y[C*3+2]-A,j=y[m*3]-P,G=y[m*3+1]-v,U=y[m*3+2]-A,[ct,lt,ut]=Re([N*U-Y*G,Y*j-D*U,D*G-N*j]),ie=De+(1-De)*Math.max(0,ct*s+lt*r+ut*i),ht=(e.faceColour[b*3]??0)*ie,dt=(e.faceColour[b*3+1]??0)*ie,mt=(e.faceColour[b*3+2]??0)*ie,pt=Math.max(0,Math.floor(Math.min(x,R,k))),ft=Math.min(t-1,Math.ceil(Math.max(x,R,k))),gt=Math.max(0,Math.floor(Math.min($,W,S))),yt=Math.min(t-1,Math.ceil(Math.max($,W,S)));for(let ee=gt;ee<=yt;ee+=1)for(let te=pt;te<=ft;te+=1){const ce=te+.5,le=ee+.5,wt=(R-x)*(le-$)-(W-$)*(ce-x),xe=(k-R)*(le-W)-(S-W)*(ce-R),ke=(x-k)*(le-S)-($-S)*(ce-k);if(wt>0||xe>0||ke>0)continue;const $e=xe/I,Te=ke/I,Se=M*$e+O*Te+L*(1-$e-Te),z=ee*t+te;Se<=a[z]||(a[z]=Se,o[z*4]=ht,o[z*4+1]=dt,o[z*4+2]=mt,o[z*4+3]=255)}}return o}const ge={levels:4,roughness:.28,share:.55},be="header-world";function tt(){try{const e=localStorage.getItem(be);if(!e)return null;const t=JSON.parse(e);return[t.seed,t.levels,t.roughness,t.share].every(o=>typeof o=="number"&&Number.isFinite(o))?t:null}catch{return null}}function Gt(e){try{localStorage.setItem(be,JSON.stringify(e))}catch{}}function Ut(){try{localStorage.removeItem(be)}catch{}}const Q=32;let de=null;function Pe(e,t){const n=document.querySelector('link[rel="icon"]');if(!n)return;de??=Object.assign(document.createElement("canvas"),{width:Q,height:Q});const o=de.getContext("2d");if(!o)return;const a=o.createImageData(Q,Q);a.data.set(ve(e,Q,{rotation:t})),o.putImageData(a,0,0),n.type="image/png",n.href=de.toDataURL("image/png")}const zt=90,Jt=400,me=new WeakMap;function ye(e,t){me.get(e)?.();const n=e.getContext("2d");if(!n)return()=>{};const o=t??{...ge,seed:Math.floor(Math.random()*16777215)},a=e.width,s=et(o),r=n.createImageData(a,a);e.dataset.seed=String(o.seed),e.title=`World ${o.seed}, ${s.mesh.faceCount.toLocaleString("en")} triangles`;const i=g=>{r.data.set(ve(s,a,{rotation:g})),n.putImageData(r,0,0),e.classList.add("grown")};if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return i(.6),Pe(s,.6),me.set(e,()=>{}),()=>{};let l=0,c=-1/0;const d=performance.now(),u=g=>{const f=(g-d)/1e3/zt*Math.PI*2;i(f),g-c>Jt&&(Pe(s,f),c=g),l=requestAnimationFrame(u)};l=requestAnimationFrame(u);const p=()=>cancelAnimationFrame(l);return me.set(e,p),p}const J=360,Xt=60,Vt=480,Zt=900,X={x:1600,y:1e3},Qt=1.4,Fe=Math.PI*2/Xt,We=Math.PI*4;function en(e){const t=h("canvas",{class:"world",width:J,height:J}),n=t.getContext("2d");if(!n)return()=>{};const o={...ge,seed:Math.floor(Math.random()*16777215)},a=n.createImageData(J,J),s=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let r,i=.6,l=-.38,c=!s,d=null,u=0,p=performance.now();const g=document.documentElement,f=g.dataset.sky==="stars";let y=0,w=0,b="";f&&g.classList.add("sky-driven");const T=(v,A=0)=>{if(!f)return;const D=Zt/(Math.PI*2);y=((y+v*D)%X.x+X.x)%X.x,w=((w-A*D)%X.y+X.y)%X.y;const N=`${(Math.round(y*2)/2).toFixed(1)}px ${(Math.round(w*2)/2).toFixed(1)}px`;if(N===b)return;b=N;const[Y,j]=N.split(" ");g.style.setProperty("--sky-x",Y??"0px"),g.style.setProperty("--sky-y",j??"0px")},C=h("p",{class:"hint"}),m=document.querySelector("canvas.planet"),x=(20*4**ge.levels).toLocaleString("en"),$=()=>{r=et(o);const v=tt();C.textContent=`World ${o.seed}: ${r.mesh.faceCount.toLocaleString("en")} triangles. `+(v?`The header is keeping world ${v.seed}, ${(20*4**v.levels).toLocaleString("en")} triangles.`:`The header grows a new one every visit, ${x} triangles each.`),R.hidden=!v,W()},M=h("button",{type:"button",onclick:()=>{Gt({...o}),m&&ye(m,{...o}),$()}},"Put it in the header"),R=h("button",{type:"button",hidden:!0,onclick:()=>{Ut(),m&&ye(m),$()}},"Let the header grow its own"),W=()=>{a.data.set(ve(r,J,{rotation:i,tilt:l})),n.putImageData(a,0,0)};let O=0;const k=v=>{const A=Math.min(.1,(v-p)/1e3);if(!d){if(u!==0){u*=Math.exp(-A/Qt);const N=c?Fe:0;(Math.abs(u)<=N||Math.abs(u)<.01)&&(u=0)}const D=u!==0?u:c?Fe:0;D!==0&&(i+=D*A,W()),T((u!==0?u:0)*A+A/Vt*Math.PI*2)}p=v,O=requestAnimationFrame(k)};t.addEventListener("pointerdown",v=>{d={x:v.clientX,y:v.clientY,at:v.timeStamp},u=0,t.setPointerCapture(v.pointerId)}),t.addEventListener("pointermove",v=>{if(!d)return;const A=t.clientWidth||J,D=(v.clientX-d.x)/A*Math.PI;i+=D;const N=l;l=Math.max(-1.2,Math.min(1.2,l-(v.clientY-d.y)/A*Math.PI)),T(D,l-N);const Y=Math.max(.004,(v.timeStamp-d.at)/1e3);u=Math.max(-We,Math.min(We,u*.4+D/Y*.6)),d={x:v.clientX,y:v.clientY,at:v.timeStamp},W()}),t.addEventListener("pointerup",v=>{d&&v.timeStamp-d.at>120&&(u=0),d=null,p=performance.now()}),t.addEventListener("pointercancel",()=>{d=null,u=0});const S=h("input",{type:"number",min:0,value:o.seed,onchange:()=>{o.seed=Math.max(0,Math.floor(Number(S.value)||0)),$()}}),L=h("button",{type:"button",onclick:()=>{o.seed=Math.floor(Math.random()*16777215),S.value=String(o.seed),$()}},"Another world"),I=h("button",{type:"button",onclick:()=>{c=!c,I.textContent=c?"Hold still":"Turn"}},c?"Hold still":"Turn"),P=(v,A,D,N,Y,j)=>{const G=h("output",{},j(o[v])),U=h("input",{type:"range",min:D,max:N,step:Y,value:o[v],onchange:()=>{o[v]=Number(U.value),G.textContent=j(o[v]),$()},oninput:()=>{G.textContent=j(Number(U.value))}});return h("label",{},`${A}: `,G,U)};return e.append(t,h("div",{class:"row"},h("span",{},"Seed "),S,L,I,M,R),h("div",{class:"dials"},P("levels","Detail",2,5,1,v=>`${v} splits`),P("roughness","Roughness",.05,.5,.01,v=>v.toFixed(2)),P("share","Sea",.2,.85,.01,v=>`${Math.round(v*100)}%`)),C),$(),O=requestAnimationFrame(k),()=>{cancelAnimationFrame(O),g.classList.remove("sky-driven"),g.style.removeProperty("--sky-x"),g.style.removeProperty("--sky-y")}}const tn={"technical-debt":St,"developer-meetings":kt,worlds:en};function Oe(){const e=[];for(const t of document.querySelectorAll(".app[data-app]")){const n=tn[t.dataset.app??""],o=n?.(t);o&&e.push(o)}return()=>{for(const t of e)t()}}const nn={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"};function H(e){return e.replace(/[&<>"]/g,t=>nn[t]??t)}function on(e,t){const o=/^https?:/.test(t)?' target="_blank" rel="noopener noreferrer"':"";return`<a href="${H(t)}"${o}>${e}</a>`}const an=["large","wide"];function sn(e,t,n){const o=n&&an.includes(n)?` class="${n}"`:"";return`<img src="${H(t)}" alt="${e}"${o}>`}function rn(e){return H(e).replace(/!\[([^\]]*)\]\(([^)\s]+)(?:\s+&quot;([^&]*)&quot;)?\)/g,(t,n,o,a)=>sn(n,o,a)).replace(/\[([^\]]+)\]\(([^)\s]+)\)/g,(t,n,o)=>on(n,o)).replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*]+)\*/g,"$1<em>$2</em>").replace(/ -- /g," — ").replace(/ {2,}\n/g,"<br>").replace(/\n/g," ")}function q(e){return e.split(/(`[^`]+`)/g).map(t=>t.startsWith("`")&&t.endsWith("`")&&t.length>1?`<code>${H(t.slice(1,-1))}</code>`:rn(t)).join("")}const Le=/^(?:[-*]|\d+\.)\s/;function cn(e,t,n){if(!Le.test(e[0]??""))return!1;const o=t.slice(n).find(a=>a.trim()!=="");return o!==void 0&&Le.test(o)}function ln(e){const t=[],n=e.replace(/\r\n?/g,`
`).split(`
`);let o=[],a=!1;return n.forEach((s,r)=>{if(s.startsWith("```")){a=!a,o.push(s),a||(t.push(o),o=[]);return}if(!a&&s.trim()===""){if(cn(o,n,r+1))return;o.length&&t.push(o),o=[];return}o.push(s)}),o.length&&t.push(o),t}function un(e){return e.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"").replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")}function hn(e){const t=/^(#{1,4})\s+(.*)$/.exec(e[0]??"");if(!t||!e.slice(0,-1).every(s=>/ {2,}$/.test(s)))return null;const o=t[1]?.length??1,a=[t[2]??"",...e.slice(1)].join(`
`);return`<h${o} id="${un(a)}">${q(a)}</h${o}>`}function dn(e){if(!e[0]?.startsWith("```"))return null;const t=e.slice(1,-1).join(`
`);return`<pre><code>${H(t)}</code></pre>`}function mn(e,t){const n=[];for(const o of e)t.test(o)?n.push(o.replace(t,"")):n.length&&(n[n.length-1]+=`
${o.trim()}`);return n}function pn(e){const t=e[0]??"",n=/^\d+\.\s/.test(t),o=/^[-*]\s/.test(t);if(!n&&!o)return null;const a=n?/^\d+\.\s+/:/^[-*]\s+/;if(!e.every(i=>a.test(i)||/^\s/.test(i)))return null;const s=n?"ol":"ul",r=mn(e,a).map(i=>`<li>${q(i)}</li>`).join("");return`<${s}>${r}</${s}>`}function fn(e){return e.every(n=>n.includes(" :: "))?`<dl>${e.map(n=>{const o=n.indexOf(" :: ");return[n.slice(0,o),n.slice(o+4)]}).map(([n,o])=>`<dt>${q(n)}</dt><dd>${q(o)}</dd>`).join("")}</dl>`:null}function gn(e){if(!e.every(n=>n.startsWith(">")))return null;const t=e.map(n=>n.replace(/^>\s?/,"")).join(" ");return`<blockquote>${q(t)}</blockquote>`}function yn(e){const t=/^::([a-z0-9-]+)$/.exec(e[0]??"");return!t||e.length!==1?null:`<div class="app" data-app="${t[1]}"></div>`}function wn(e){return e.length===1&&/^-{3,}$/.test(e[0]??"")?"<hr>":null}function vn(e){const t=e.length===1&&/^(\\+)$/.exec(e[0]??"");return t?`<div class="space" style="--n:${t[1]?.length??1}"></div>`:null}function bn(e){return e.length===1&&/^!\[[^\]]*\]\([^)\s]+(?:\s+"[^"]*")?\)$/.test(e[0]??"")?`<figure>${q(e[0]??"")}</figure>`:null}function xn(e){return`<p>${q(e.join(`
`))}</p>`}const kn=[wn,vn,hn,dn,gn,yn,bn,fn,pn];function nt(e){return ln(e).map(t=>{for(const n of kn){const o=n(t);if(o!==null)return o}return xn(t)}).join(`
`)}function $n(e,t){if(t.parent===null)return"";const n=e.childrenOf(t.route);return n.length===0?"":`<ul class="listing">${n.map(a=>`<li><a href="${a.route}">${H(a.title)}</a>`+(a.summary?` <span class="summary">${H(a.summary)}</span>`:"")+"</li>").join("")}</ul>`}function Tn(e,t){const n=e.trailTo(t.route);if(n.length<=1)return"";const o=n.slice(1).map(a=>a.name).join("/");return`<p class="ran"><span class="ps1">~ $</span> cd ${H(o)} &amp;&amp; cat *</p>`}function Sn(e,t){return`${Tn(e,t)}
${nt(t.body)}
${$n(e,t)}`}const Mn="theme";function ot(){const e=document.documentElement,t=e.dataset.pageTheme;let n=null;try{n=localStorage.getItem(Mn)}catch{n=null}const o=t??(n==="light"||n==="dark"?n:null);o?e.dataset.theme=o:delete e.dataset.theme}function En(e,t){const n=document.querySelector("main");if(!n)return()=>!1;const o=(a,s=!0)=>{const r=e.at(a);if(!r)return!1;n.innerHTML=Sn(e,r);const i=document.documentElement,l=r.fields.theme;l==="dark"||l==="light"?i.dataset.pageTheme=l:delete i.dataset.pageTheme,r.fields.sky?i.dataset.sky=r.fields.sky:delete i.dataset.sky,ot(),document.title=r.route==="/"?"David Rodenas":`${r.title} — David Rodenas`;for(const c of document.querySelectorAll("nav .navlink"))a.startsWith(c.getAttribute("href")??"\0")?c.setAttribute("aria-current","page"):c.removeAttribute("aria-current");return s&&(window.history.pushState({route:a},"",a),window.scrollTo({top:0})),window.goatcounter?.count?.({path:a,title:document.title}),t(a),!0};return document.addEventListener("click",a=>{if(a.defaultPrevented||a.button!==0||a.metaKey||a.ctrlKey||a.shiftKey||a.altKey)return;const s=a.target?.closest("a[href]");if(!s||s.target||s.dataset.run)return;const r=new URL(s.href,window.location.href);if(r.origin!==window.location.origin)return;const i=r.pathname.endsWith("/")?r.pathname:`${r.pathname}/`;e.at(i)&&(a.preventDefault(),i!==window.location.pathname&&o(i))}),window.addEventListener("popstate",()=>{const a=window.location.pathname.endsWith("/")?window.location.pathname:`${window.location.pathname}/`;o(a,!1)}),o}const Cn=[{file:"book/index.md",markdown:`---
...
nothing to paint.
`}],Ne="---";function An(e){return(/^"(.*)"$/.exec(e)??/^'(.*)'$/.exec(e))?.[1]??e}function In(e){const t=e.replace(/\r\n?/g,`
`).split(`
`);if(t[0]?.trim()!==Ne)return{fields:{},body:e.trim()};const n=t.indexOf(Ne,1);if(n<0)return{fields:{},body:e.trim()};const o={};for(const a of t.slice(1,n)){const s=a.indexOf(":");s<=0||(o[a.slice(0,s).trim()]=An(a.slice(s+1).trim()))}return{fields:o,body:t.slice(n+1).join(`
`).trim()}}function Dn(e){const n=e.replace(/\.md$/,"").replace(/(^|\/)index$/,"");return n===""?"/":`/${n}/`}function Rn(e){if(e==="/")return null;const t=e.slice(0,-1);return t.slice(0,t.lastIndexOf("/")+1)}function _e(e){if(e==="/")return"/";const t=e.slice(0,-1);return t.slice(t.lastIndexOf("/")+1)}function Pn(e){const{fields:t,body:n}=In(e.markdown),o=Dn(e.file);return{file:e.file,route:o,parent:Rn(o),name:_e(o),title:t.title??_e(o),summary:t.summary??"",order:Number(t.order??"100"),body:n,fields:t}}function He(e,t){return e.order-t.order||e.name.localeCompare(t.name)}class Fn{byRoute;constructor(t){const n=t.map(Pn).sort(He);this.byRoute=new Map(n.map(o=>[o.route,o]))}get pages(){return[...this.byRoute.values()]}at(t){return this.byRoute.get(t)}childrenOf(t){return this.pages.filter(n=>n.parent===t).sort(He)}trailTo(t){const n=this.at(t);return n?n.parent===null?[n]:[...this.trailTo(n.parent),n]:[]}}const pe=new Fn(Cn);class Wn{lines=[];drafts=[];index=0;add(t){this.lines.push(t),this.drafts=[...this.lines,""],this.index=this.lines.length}previous(t){return this.moveTo(this.index-1,t)}next(t){return this.moveTo(this.index+1,t)}moveTo(t,n){return this.drafts.length===0&&(this.drafts=[""]),t<0||t>=this.drafts.length?n:(this.drafts[this.index]=n,this.index=t,this.drafts[t]??n)}}function at(e){return e.split(/\s*(?:;|&&)\s*/).map(t=>t.trim().split(/\s+/).filter(Boolean)).filter(t=>t.length>0)}function re(e,t){const o=t.startsWith("~")||t.startsWith("/")?[]:e.split("/").filter(Boolean),a=t.replace(/^~/,"").split("/").filter(Boolean),s=[...o];for(const r of a)r!=="."&&(r===".."?s.pop():s.push(r));return s.length===0?"/":`/${s.join("/")}/`}function On(e){return e.replace(/(?:^|\/)(?:README\.md|\*)$/,"")||"."}const Ln={name:"cat",usage:"cat <file>",description:"print a page, README.md or * for the one here",run({site:e,cwd:t},[n]){if(!n)return{text:"cat: usage: cat <file>",error:!0};const o=e.at(re(t,On(n)));return!o||/\.md$/.test(n)!==/README\.md$/.test(n)?{text:`cat: ${n}: no such file`,error:!0}:{html:nt(o.body)}}},Nn={name:"cd",usage:"cd [dir]",description:"go to a directory (the page follows)",run(e,[t="~"]){const n=re(e.cwd,t);return e.site.at(n)?(e.cwd=n,{navigate:n}):{text:`cd: ${t}: no such directory`,error:!0}}},_n={name:"clear",usage:"clear",description:"clear what the shell has printed",run(){return{clear:!0}}},Hn={name:"help",usage:"help [command]",description:"this",run({commands:e},[t]){if(t){const a=e.find(s=>s.name===t);return a?{text:`${a.usage}
...
`)}}};function Yn(e){const t=e.filter(o=>o.startsWith("-")).flatMap(o=>o.slice(1).split("")),n=e.find(o=>!o.startsWith("-"))??".";return{flags:t,path:n}}function jn(e,t,n){const o=n==="."?"":`${n.replace(/\/$/,"")}/`;return[{mode:"--r-",name:"README.md",title:e.title,href:e.route,run:`cat ${o}README.md`},...t.map(a=>({mode:"dr-x",name:`${a.name}/`,title:a.title,href:a.route}))]}function Ye(e){const t=e.run?` data-run="${H(e.run)}"`:"";return`<a href="${H(e.href)}"${t}>${H(e.name)}</a>`}function Bn(e,t){const n=(s,r)=>t?`${s.mode}  ${r.padEnd(20)}  ${s.title}`:r,o=s=>t?`${s.mode}  ${Ye(s)}${" ".repeat(Math.max(0,20-s.name.length))}  ${H(s.title)}`:Ye(s),a=t?[`total ${e.length}`]:[];return{text:[...a,...e.map(s=>n(s,s.name))].join(`
`),html:`<pre>${[...a,...e.map(o)].join(`
`)}</pre>`}}const Kn={name:"ls",usage:"ls [-l] [path]",description:"list what a directory holds; -l says what each is",run({site:e,cwd:t},n){const{flags:o,path:a}=Yn(n),s=o.find(l=>l!=="l");if(s)return{text:`ls: -${s}: no such option. Try ls -l`,error:!0};const r=re(t,a),i=e.at(r);return i?Bn(jn(i,e.childrenOf(r),a),o.includes("l")):{text:`ls: ${a}: no such directory`,error:!0}}};function st(e){return e==="/"?"~":`~${e.replace(/\/$/,"")}`}const qn={name:"pwd",usage:"pwd",description:"print where you are",run({cwd:e}){return{text:st(e)}}},Gn=["light","dark","system"];function Un(e){return Gn.includes(e)}const zn={name:"theme",usage:"theme [light|dark|system|auto]",description:"switch the colours, or toggle them",run({site:e,cwd:t},[n]){const o=e.at(t)?.fields.theme;if(o)return{text:`theme: this page keeps its own, ${o}. It works everywhere else.`,error:!0};if(n===void 0)return{theme:"toggle",text:"theme: toggled"};const a=n==="auto"?"system":n;return Un(a)?{theme:a,text:`theme: ${a}`}:{text:`theme: ${n}: choose light, dark or system`,error:!0}}},Jn=[Kn,Nn,Ln,qn,Hn,_n,zn];class Xn{context;constructor(t,n,o=Jn){this.context={site:t,cwd:n,commands:o}}get prompt(){return`${st(this.context.cwd)} $`}moveTo(t){return this.context.site.at(t)?(this.context.cwd=t,!0):!1}run(t){const n=[];for(const[o="",...a]of at(t)){const s=this.context.commands.find(i=>i.name===o),r=s?s.run(this.context,a):{text:`${o}: command not found. Try help`,error:!0};if(n.push(r),r.error)break}return n}complete(t){const n=t.split(/\s+/),o=n.pop()??"",a=n.length===0?"":`${n.join(" ")} `;return(n.length===0?this.commandNames():this.pathNames(o)).filter(r=>r.startsWith(o)).map(r=>a+r)}commandNames(){return this.context.commands.map(t=>t.name).sort()}pathNames(t){const n=t.lastIndexOf("/"),o=n<0?".":t.slice(0,n+1),a=re(this.context.cwd,o);if(!this.context.site.at(a))return[];const s=n<0?"":o;return["README.md",...this.context.site.childrenOf(a).map(i=>`${i.name}/`)].map(i=>s+i)}}const we="theme";function Vn(){return window.matchMedia("(prefers-color-scheme: dark)").matches}function Zn(){let e=null;try{e=localStorage.getItem(we)}catch{e=document.documentElement.dataset.theme??null}return e==="light"||e==="dark"?e:Vn()?"dark":"light"}function Qn(e){const t=e==="toggle"?Zn()==="dark"?"light":"dark":e;try{t==="system"?localStorage.removeItem(we):localStorage.setItem(we,t)}catch{}return ot(),t}const rt="shell-screen",it="shell-pending";function eo(e,t){try{sessionStorage.setItem(rt,e.innerHTML),t&&sessionStorage.setItem(it,t)}catch{}}function je(e){try{const t=sessionStorage.getItem(e)??"";return sessionStorage.removeItem(e),t}catch{return""}}function to(){window.__stopTyped?.();const e=window.__typed??[];if(window.__typed=[],e.length===0)return null;const t=[];let n="";for(const o of e)o==="Enter"?(t.push(n),n=""):o==="Backspace"?n=n.slice(0,-1):n+=o;return{finished:t,unfinished:n}}function no(e,t,n={}){const o=document.querySelector(".terminal"),a=o?.querySelector(".screen"),s=o?.querySelector("form.prompt"),r=s?.querySelector("input"),i=s?.querySelector(".ps1");if(!o||!a||!s||!r||!i)return null;const l=new Xn(e,t),c=new Wn;let d=null;const u=m=>{a.append(m)},p=()=>{d?.remove(),d=null},g=m=>{if(m.clear&&a.replaceChildren(),m.theme){const x=Qn(m.theme);u(h("pre",{},`theme: ${x}`));return}if(m.html){const x=h("div",{class:m.text?"listing-out":"cat"});x.innerHTML=m.html,u(x)}else m.text&&u(h("pre",{class:m.error?"error":""},m.text))},f=m=>{p(),u(h("p",{class:"echo"},h("span",{class:"ps1"},l.prompt),` ${m}`));const x=at(m).map($=>$.join(" "));for(let $=0;$<x.length;$+=1){const[M]=l.run(x[$]??"");if(M){if(M.navigate){if(n.navigate?.(M.navigate))continue;eo(a,x.slice($+1).join(" && ")),window.location.assign(M.navigate);return}if(g(M),M.error)break}}i.textContent=l.prompt,r.scrollIntoView({block:"nearest"})},y=()=>{if(p(),r.value.trim()===""){r.value="help";return}const m=l.complete(r.value);m.length===1?r.value=m[0]??r.value:m.length>1&&(d=h("p",{class:"hint"},m.map(x=>x.split(" ").pop()).join("  ")),u(d))};s.addEventListener("submit",m=>{m.preventDefault();const x=r.value.trim();r.value="",x&&(c.add(x),f(x))}),r.addEventListener("keydown",m=>{m.key==="Tab"?(m.preventDefault(),y()):m.key==="ArrowUp"?(m.preventDefault(),r.value=c.previous(r.value)):m.key==="ArrowDown"?(m.preventDefault(),r.value=c.next(r.value)):p()}),a.addEventListener("click",m=>{const x=m.target?.closest("a[data-run]");x?.dataset.run&&(m.preventDefault(),f(x.dataset.run))}),window.addEventListener("keydown",m=>{const $=m.target?.matches("input, textarea, select, [contenteditable]")??!1,M=m.key.length===1&&!m.ctrlKey&&!m.metaKey&&!m.altKey;$||!M||r.focus({preventScroll:!1})}),o.hidden=!1;const w=je(rt);w&&(a.innerHTML=w);const b=je(it);b&&f(b);const T=to();if(T){for(const m of T.finished)m.trim()&&(c.add(m.trim()),f(m.trim()));r.value=T.unfinished,r.focus()}return{run:f,moveTo:m=>{l.moveTo(m)&&(i.textContent=l.prompt)}}}function Be(){const e=document.querySelector("canvas.planet");e&&ye(e,tt()??void 0);const t=window.location.pathname.endsWith("/")?window.location.pathname:`${window.location.pathname}/`;let n=Oe(),o=null;const a=En(pe,r=>{n(),n=Oe(),o?.moveTo(r)});o=no(pe,pe.at(t)?t:"/",{navigate:a});const s=document.querySelector(".theme-toggle");s&&o&&(s.classList.add("ready"),s.removeAttribute("aria-hidden"),s.removeAttribute("tabindex"),s.addEventListener("click",()=>o?.run("theme")))}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",Be):Be();
