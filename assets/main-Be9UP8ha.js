...
`);return`<h${a} id="${Pr(s)}">${V(s)}</h${a}>`}function Rr(e){if(!e[0]?.startsWith("```"))return null;const t=e[0].slice(3).trim(),n=e.slice(1,-1).join(`
`);return t==="flow"?vr(n):`<pre><code>${Tr(n,t)}</code></pre>`}function Fr(e,t){const n=[];for(const a of e)t.test(a)?n.push(a.replace(t,"")):n.length&&(n[n.length-1]+=`
${a.trim()}`);return n}function Lr(e){const t=e[0]??"",n=/^\d+\.\s/.test(t),a=/^[-*]\s/.test(t);if(!n&&!a)return null;const s=n?/^\d+\.\s+/:/^[-*]\s+/;if(!e.every(i=>s.test(i)||/^\s/.test(i)))return null;const o=n?"ol":"ul",r=Fr(e,s).map(i=>`<li>${V(i)}</li>`).join("");return`<${o}>${r}</${o}>`}function Wr(e){return e.every(n=>n.includes(" :: "))?`<dl>${e.map(n=>{const a=n.indexOf(" :: ");return[n.slice(0,a),n.slice(a+4)]}).map(([n,a])=>`<dt>${V(n)}</dt><dd>${V(a)}</dd>`).join("")}</dl>`:null}function Br(e){if(!e.every(n=>n.startsWith(">")))return null;const t=e.map(n=>n.replace(/^>\s?/,"")).join(" ");return`<blockquote>${V(t)}</blockquote>`}function Hr(e){const t=/^::([a-z0-9-]+)$/.exec(e[0]??"");return!t||e.length!==1?null:`<div class="app" data-app="${t[1]}"></div>`}function Jr(e){return e.length===1&&/^-{3,}$/.test(e[0]??"")?"<hr>":null}function _r(e){const t=e.length===1&&/^(\\+)$/.exec(e[0]??"");return t?`<div class="space" style="--n:${t[1]?.length??1}"></div>`:null}function Gr(e){return e.length===1&&/^!\[[^\]]*\]\([^)\s]+(?:\s+"[^"]*")?\)$/.test(e[0]??"")?`<figure>${V(e[0]??"")}</figure>`:null}function qr(e){return`<p>${V(e.join(`
`))}</p>`}const Ur=[Jr,_r,Nr,Rr,Br,Hr,Gr,Wr,Lr];function ea(e){return Or(e).map(t=>{for(const n of Ur){const a=n(t);if(a!==null)return a}return qr(t)}).join(`
