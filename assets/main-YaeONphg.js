const K=t=>Math.max(0,Math.min(100,t));function je(t){const{focus:e,fatigue:n,featureSize:o,weeks:a,calendar:s,meetingTypes:r}=t,c=[];let l=0,i=0;for(let d=0;d<a;d+=1)for(let u=0;u<5;u+=1){let p=0,w=0;for(let g=0;g<8;g+=1){const k=r[s[`${u}-${g}`]??""];if(k){p=K(p+k.focus),w=K(w+k.fatigue),c.push({week:d,day:u,hour:g,inMeeting:!0,hourFocus:p,hourFatigue:w,hourProductivity:0,accumulatedProductivity:l,completedFeatures:i,featureCompleted:!1});continue}p=K(p+e),w=K(w+n);const x=K(p-w),M=o-l,A=x>M,D=A?M:x;A?(i+=1,l=0):l+=D,c.push({week:d,day:u,hour:g,inMeeting:!1,hourFocus:p,hourFatigue:w,hourProductivity:D,accumulatedProductivity:l,completedFeatures:i,featureCompleted:A}),A&&(p=0)}}return c}function Z(){return Array.from({length:8},()=>new Array(5).fill(0))}function Q(t,{hour:e,day:n},o){const a=t[e];a&&(a[n]=(a[n]??0)+o)}function He(t,{featureSize:e,weeks:n}){const o=t[t.length-1],a=o?.completedFeatures??0,s=o?.accumulatedProductivity??0,r=a+Math.round(10*s/e)/10,c=a*e+s,l=Array.from({length:5},()=>({productivity:0,features:0,meetings:0})),i={focus:Z(),fatigue:Z(),productivity:Z(),features:Z()};for(const u of t){const p=l[u.day];p.productivity+=u.hourProductivity,u.featureCompleted&&(p.features+=1),u.inMeeting&&(p.meetings+=1),Q(i.focus,u,u.hourFocus),Q(i.fatigue,u,u.hourFatigue),Q(i.productivity,u,u.hourProductivity),u.featureCompleted&&Q(i.features,u,1)}const d=u=>u.map(p=>p.map(w=>n>0?w/n:0));return{totalFeatures:r,totalProductivity:c,averageFeaturesPerWeek:n>0?r/n:0,averageProductivityPerWeek:n>0?c/n:0,days:l,hours:{focus:d(i.focus),fatigue:d(i.fatigue),productivity:d(i.productivity),features:i.features}}}const oe=480,H=240,I={top:10,right:10,bottom:34,left:36};function rt(t,e,n,o){const a=oe-I.left-I.right,s=H-I.top-I.bottom,r=i=>I.top+s-(t>0?i/t*s:0),c=o.map(i=>`<line class="grid" x1="${I.left}" x2="${oe-I.right}" y1="${r(i)}" y2="${r(i)}"/><text x="${I.left-4}" y="${r(i)+3}" text-anchor="end">${i}</text>`).join(""),l=(n>1?[1,Math.ceil(n/2),n]:[]).filter((i,d,u)=>u.indexOf(i)===d).map(i=>`<text x="${I.left+(i-1)/Math.max(1,n-1)*a}" y="${H-I.bottom+14}" text-anchor="middle">${i}</text>`).join("");return`${c}${l}<text x="${I.left+a/2}" y="${H-6}" text-anchor="middle">${e.x}</text><text transform="translate(9 ${I.top+s/2}) rotate(-90)" text-anchor="middle">${e.y}</text>`}function it(t){if(t<=0)return[0];const e=10**Math.floor(Math.log10(t)),n=t/e>=5?e:t/e>=2?e/2:e/5,o=[];for(let a=0;a<=t;a+=n)o.push(Math.round(a*100)/100);return o}function Nt(t,e){const n=Math.max(...t.map(d=>d.values.length),1),o=Math.max(1,...t.flatMap(d=>d.values)),a=oe-I.left-I.right,s=H-I.top-I.bottom,r=d=>I.left+d/Math.max(1,n-1)*a,c=d=>I.top+s-d/o*s,l=t.map(d=>{const u=d.values.map((p,w)=>`${r(w).toFixed(1)},${c(p).toFixed(1)}`).join(" ");return`<polyline class="line ${d.className}" points="${u}"><title>${d.name}</title></polyline>`}).join(""),i=t.map((d,u)=>`<rect class="${d.className}" x="${I.left+u*90}" y="${H-I.bottom+20}" width="10" height="3"/><text x="${I.left+u*90+14}" y="${H-I.bottom+24}">${d.name}</text>`).join("");return`<svg viewBox="0 0 ${oe} ${H}" role="img" aria-label="${e.y} by ${e.x}">${rt(o,e,n,it(o))}${l}${i}</svg>`}const _e=480,Y=240,N={top:10,right:10,bottom:34,left:36};function ct(t,e,n){const o=Math.max(...t.map(g=>g.values.length),1),a=Math.max(1,...t.flatMap(g=>g.values)),s=_e-N.left-N.right,r=Y-N.top-N.bottom,c=s/o,l=c*.7/t.length,i=g=>N.top+r-g/a*r,d=t.map((g,k)=>g.values.map((x,M)=>{const A=N.left+M*c+c*.15+k*l;return`<rect class="${g.className}" x="${A.toFixed(1)}" y="${i(x).toFixed(1)}" width="${l.toFixed(1)}" height="${(N.top+r-i(x)).toFixed(1)}"><title>${g.name}: ${Math.round(x*10)/10}</title></rect>`}).join("")).join(""),u=(n??[]).map((g,k)=>`<text x="${N.left+k*c+c/2}" y="${Y-N.bottom+14}" text-anchor="middle">${g}</text>`).join(""),p=t.map((g,k)=>`<rect class="${g.className}" x="${N.left+k*90}" y="${Y-N.bottom+20}" width="10" height="3"/><text x="${N.left+k*90+14}" y="${Y-N.bottom+24}">${g.name}</text>`).join(""),w=rt(a,e,n?0:o,it(a));return`<svg viewBox="0 0 ${_e} ${Y}" role="img" aria-label="${e.y} by ${e.x}">${w}${d}${u}${p}</svg>`}function h(t,e={},...n){const o=document.createElement(t);for(const[a,s]of Object.entries(e))s===void 0||s===!1||(typeof s=="function"?o.addEventListener(a.slice(2).toLowerCase(),s):s===!0?o.setAttribute(a,""):o.setAttribute(a,String(s)));for(const a of n)a==null||a===!1||o.append(a);return o}const ge=["Mon","Tue","Wed","Thu","Fri"],lt=Array.from({length:8},(t,e)=>`${9+e}:00`);function Bt(t){return t<=500?t:t<=750?500+(t-500)*2:t<1e3?1e3+(t-750)*35:1e4}function Lt(t){return t<=500?t:t<=1e3?500+(t-500)/2:t<1e4?750+(t-1e3)/35:1e3}function ee(t,e){const n=e.flat(),o=Math.min(...n),a=Math.max(...n),s=h("div",{class:"week"},h("span"),...ge.map(r=>h("span",{class:"head"},r)));return e.forEach((r,c)=>{s.append(h("span",{class:"hour"},lt[c]??""));for(const l of r){const i=a>o?(l-o)/(a-o):0;s.append(h("span",{class:"cell",style:`--heat:${(.1+i*.9).toFixed(2)}`},String(Math.round(l))))}}),h("div",{},h("h4",{},t),s)}function jt(t){const e={focus:25,fatigue:15,featureSize:300,weeks:8},n={"🍽️ Lunch":{focus:-100,fatigue:-100},"🏃 Sprint plan":{focus:-100,fatigue:50},"😴 Boring":{focus:-50,fatigue:-25}},o={};for(let b=0;b<5;b+=1)o[`${b}-3`]="🍽️ Lunch";let a="🏃 Sprint plan",s=null;const r=h("div",{class:"figures"}),c=h("div",{class:"chart"}),l=h("div",{class:"maps"}),i=h("div",{class:"week"}),d=h("select"),u=h("input",{type:"number",min:-100,max:100}),p=h("input",{type:"number",min:-100,max:100}),w=h("input",{type:"text",placeholder:"New meeting name",size:16}),g=(b,f,$,S,E=P=>P,L=P=>P)=>{const P=h("output",{},String(e[b])),W=h("input",{type:"range",min:$,max:S,value:L(e[b]),oninput:()=>{e[b]=E(Number(W.value)),P.textContent=String(e[b]),C()}});return h("label",{},`${f}: `,P,W)},k=h("div",{class:"dials"},g("focus","Focus an hour",0,100),g("fatigue","Fatigue an hour",0,100),g("featureSize","Feature size",0,1e3,Bt,Lt),g("weeks","Weeks",1,16));function x(){d.replaceChildren(...Object.keys(n).map(f=>h("option",{value:f,selected:f===a},f)));const b=n[a];u.value=String(b?.focus??0),p.value=String(b?.fatigue??0)}d.addEventListener("change",()=>{a=d.value,x()});const M=()=>{n[a]={focus:Number(u.value)||0,fatigue:Number(p.value)||0},C()};u.addEventListener("change",M),p.addEventListener("change",M);const A=()=>{const b=w.value.trim();!b||n[b]||(n[b]={focus:0,fatigue:0},a=b,w.value="",x())},D=h("div",{class:"row"},h("span",{},"Paint: "),d,h("span",{},"focus "),u,h("span",{},"fatigue "),p,w,h("button",{type:"button",onclick:A},"Add"));let R=null;const m=b=>{if(R==="add"&&!o[b])o[b]=a;else if(R==="remove"&&o[b])delete o[b];else return;C()};function v(){i.replaceChildren(h("span"),...ge.map(b=>h("span",{class:"head"},b))),lt.forEach((b,f)=>{i.append(h("span",{class:"hour"},b));for(let $=0;$<5;$+=1){const S=`${$}-${f}`,E=o[S];i.append(h("span",{class:E?"slot meeting":"slot",title:E??"free",onpointerdown:L=>{L.preventDefault(),R=o[S]?"remove":"add",m(S)},onpointerenter:()=>{R&&m(S)}},E?E.slice(0,2):""))}})}window.addEventListener("pointerup",()=>{R=null});const y=h("div",{class:"row"}),T=()=>{s={summary:He(je({...e,calendar:o,meetingTypes:n}),e),weeks:e.weeks},C()},F=()=>{s=null,C()};function C(){v();const b=je({...e,calendar:o,meetingTypes:n}),f=He(b,e),$=e.weeks*5*8;r.replaceChildren(h("div",{class:"clean"},h("strong",{},f.totalFeatures.toFixed(1)),"features finished"),h("div",{},h("strong",{},f.averageFeaturesPerWeek.toFixed(2)),"features a week"),h("div",{},h("strong",{},Math.round(f.totalProductivity/$).toString()),"productivity an hour"),h("div",{},h("strong",{},String($)),"hours simulated")),y.replaceChildren(s?h("span",{},`Baseline: ${s.summary.averageFeaturesPerWeek.toFixed(2)} features a week over ${s.weeks} weeks; now ${f.averageFeaturesPerWeek.toFixed(2)}. `):h("span",{},"Keep this run to compare against: "),h("button",{type:"button",onclick:T},s?"Save again":"Save as baseline")),s&&y.append(h("button",{type:"button",onclick:F},"Clear")),c.innerHTML=ct([{name:"Productivity",className:"clean",values:f.days.map(S=>S.productivity/e.weeks)},{name:"Features ×100",className:"debt",values:f.days.map(S=>S.features/e.weeks*100)}],{x:"",y:"A day, on average"},ge),c.prepend(h("h4",{},"The shape of a week")),l.replaceChildren(ee("Focus",f.hours.focus),ee("Fatigue",f.hours.fatigue),ee("Productivity",f.hours.productivity),ee("Features finished",f.hours.features))}x(),t.append(k,D,h("div",{class:"charts"},i,c),r,y,l),C()}const Ht={name:"developer-meetings",apps:{"developer-meetings":jt}};class _t{listeners=new Set;send(e){for(const n of[...this.listeners])n(e)}on(e){return this.listeners.add(e),()=>{this.listeners.delete(e)}}}const ye=new _t,Jt=900,qt=480,te={x:1600,y:1e3};function ne(t,e){return(t%e+e)%e}class Gt{x=0;y=0;written="";driving=!1;follow({byRadians:e,tiltedBy:n,seconds:o}){const a=document.documentElement;if(a.dataset.sky!=="stars")return;this.driving||this.takeOver(a);const s=Jt/(Math.PI*2),r=(o/qt*Math.PI*2+e)*s;this.x=ne(this.x+r,te.x),this.y=ne(this.y-n*s,te.y);const c=`${(Math.round(this.x*2)/2).toFixed(1)}px ${(Math.round(this.y*2)/2).toFixed(1)}px`;if(c===this.written)return;this.written=c;const[l,i]=c.split(" ");a.style.setProperty("--sky-x",l??"0px"),a.style.setProperty("--sky-y",i??"0px")}release(){const e=document.documentElement;e.classList.remove("sky-driven"),e.style.removeProperty("--sky-x"),e.style.removeProperty("--sky-y"),this.x=0,this.y=0,this.written="",this.driving=!1}takeOver(e){const n=getComputedStyle(document.body,"::before").transform;if(n&&n!=="none")try{const o=new DOMMatrixReadOnly(n);this.x=ne(o.m41,te.x),this.y=ne(o.m42,te.y)}catch{}e.classList.add("sky-driven"),this.driving=!0}}function Kt(t){return ye.on(e=>t.follow(e))}const Je=new Gt,Yt={name:"sky",install:()=>Kt(Je),arrive:()=>Je.release()},he=20;function Ut(t){const{baseTime:e,shortcutFactor:n,interestRate:o,timeHorizon:a}=t,s=[];let r=null;const c=e;let l=e*(1-n),i=0,d=0,u=0,p=0,w=0,g=0;for(let k=0;k<a*he;){for(;w<=k;)i+=1,u+=1,w+=c;for(;g<=k;)d+=1,p+=1,g+=l,l*=1+o;if(k+=1,k%he===0){const x=k/he;s.push({month:x,cleanCumulative:i,debtCumulative:d,cleanMonthly:u,debtMonthly:p,debtFeatureCost:l}),u=0,p=0,r===null&&i>d&&(r=x)}}return{months:s,breakEvenMonth:r}}const zt=[{key:"baseTime",label:"Base time",min:1,max:30,step:1,show:t=>`${t} days`},{key:"shortcutFactor",label:"Shortcuts",min:0,max:.9,step:.05,show:t=>`${Math.round(t*100)}%`},{key:"interestRate",label:"Interest",min:0,max:1,step:.01,show:t=>`${Math.round(t*100)}%`},{key:"timeHorizon",label:"Timeline",min:6,max:60,step:1,show:t=>`${t} months`}];function Vt(t){const e={baseTime:20,shortcutFactor:.25,interestRate:.1,timeHorizon:24},n=h("div",{class:"figures"}),o=h("div",{class:"chart"}),a=h("div",{class:"chart"}),s=h("p"),r=h("div",{class:"dials"},...zt.map(l=>{const i=h("output",{},l.show(e[l.key])),d=h("input",{type:"range",min:l.min,max:l.max,step:l.step,value:e[l.key],oninput:()=>{e[l.key]=Number(d.value),i.textContent=l.show(e[l.key]),c()}});return h("label",{},`${l.label}: `,i,d)}));function c(){const{months:l,breakEvenMonth:i}=Ut(e),d=l[l.length-1],u=d?.cleanCumulative??0,p=d?.debtCumulative??0,w=u>0?(u-p)/u*100:0,g=Math.abs(w)<.1?"Even":w>0?"Loss":"Gain",k=Math.abs(w)<.1?"≈0%":`${Math.abs(w).toFixed(1)}%`;n.replaceChildren(h("div",{class:"clean"},h("strong",{},String(u)),"clean features"),h("div",{class:"debt"},h("strong",{},String(p)),"debt features"),h("div",{},h("strong",{},i?`month ${i}`:"never"),"break-even"),h("div",{},h("strong",{},k),`${g.toLowerCase()} on the shortcut road`)),o.innerHTML=Nt([{name:"Clean",className:"clean",values:l.map(x=>x.cleanCumulative)},{name:"Debt-driven",className:"debt",values:l.map(x=>x.debtCumulative)}],{x:"Months",y:"Features"}),o.prepend(h("h4",{},"Cumulative features")),a.innerHTML=ct([{name:"Clean",className:"clean",values:l.slice(1).map(x=>x.cleanMonthly)},{name:"Debt-driven",className:"debt",values:l.slice(1).map(x=>x.debtMonthly)}],{x:"Months",y:"Features a month"}),a.prepend(h("h4",{},"Monthly delivery rate")),s.textContent=e.interestRate===0?"With no interest there is no compound slowdown, and the shortcut simply wins. That is the one case that does not happen to real code.":i?`${Math.round(e.shortcutFactor*100)}% saved at first, ${Math.round(e.interestRate*100)}% interest on every feature: clean development overtakes at month ${i}, and by month ${e.timeHorizon} the shortcut road has delivered ${k} less.`:`${Math.round(e.shortcutFactor*100)}% saved at first, ${Math.round(e.interestRate*100)}% interest on every feature: in ${e.timeHorizon} months the clean road has not yet caught up. Give it longer, or raise the interest.`}t.append(r,n,h("div",{class:"charts"},o,a),s),c()}const Xt={name:"technical-debt",apps:{"technical-debt":Vt}},Zt="theme";function ht(){const t=document.documentElement,e=t.dataset.pageTheme;let n=null;try{n=localStorage.getItem(Zt)}catch{n=null}const o=e??(n==="light"||n==="dark"?n:null);o?t.dataset.theme=o:delete t.dataset.theme}const we="theme";function Qt(){return window.matchMedia("(prefers-color-scheme: dark)").matches}function en(){let t=null;try{t=localStorage.getItem(we)}catch{t=document.documentElement.dataset.theme??null}return t==="light"||t==="dark"?t:Qt()?"dark":"light"}class tn{apply(e){const n=e==="toggle"?en()==="dark"?"light":"dark":e;try{n==="system"?localStorage.removeItem(we):localStorage.setItem(we,n)}catch{}return ht(),n}}function nn(t){const e=document.querySelector(".theme-toggle");return e?(e.classList.add("ready"),e.removeAttribute("aria-hidden"),e.removeAttribute("tabindex"),e.addEventListener("click",t),()=>e.removeEventListener("click",t)):()=>{}}const on={"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;"};function O(t){return t.replace(/[&<>"]/g,e=>on[e]??e)}const dt=["light","dark","system"];function an(t){return dt.includes(t)}function qe(t){const e=dt.filter(o=>o!==t),n=o=>`theme ${o}`;return{text:`theme: ${t}
  ${e.map(n).join("   ")}`,html:`<pre>theme: ${t}
  ${e.map(o=>`<a href="#" data-run="${O(n(o))}">${O(n(o))}</a>`).join("   ")}</pre>`}}function sn(t){return{name:"theme",usage:"theme [light|dark|system|auto]",description:"switch the colours, or toggle them",run({site:e,cwd:n},[o]){const a=e.at(n)?.fields.theme;if(a)return{text:`theme: this page keeps its own, ${a}. It works everywhere else.`,error:!0};if(o===void 0)return qe(t.apply("toggle"));const s=o==="auto"?"system":o;return an(s)?qe(t.apply(s)):{text:`theme: ${o}: choose light, dark or system`,error:!0}}}}const rn={name:"theme",commands:[sn(new tn)],install:t=>nn(()=>t.run("theme")),arrive:()=>ht()},ke="header-world";function ut(){try{const t=localStorage.getItem(ke);if(!t)return null;const e=JSON.parse(t);return[e.seed,e.levels,e.roughness,e.share].every(o=>typeof o=="number"&&Number.isFinite(o))?e:null}catch{return null}}function cn(t){try{localStorage.setItem(ke,JSON.stringify(t))}catch{}}function ln(){try{localStorage.removeItem(ke)}catch{}}const B=(1+Math.sqrt(5))/2,hn=[[-1,B,0],[1,B,0],[-1,-B,0],[1,-B,0],[0,-1,B],[0,1,B],[0,-1,-B],[0,1,-B],[B,0,-1],[B,0,1],[-B,0,-1],[-B,0,1]],dn=[[0,11,5],[0,5,1],[0,1,7],[0,7,10],[0,10,11],[1,5,9],[5,11,4],[11,10,2],[10,7,6],[7,1,8],[3,9,4],[3,4,2],[3,2,6],[3,6,8],[3,8,9],[4,9,5],[2,4,11],[6,2,10],[8,6,7],[9,8,1]];function un(){const t=hn.map(([e,n,o])=>{const a=Math.hypot(e,n,o);return{direction:[e/a,n/a,o/a],radius:1,surface:0}});return mt(t,dn.map(e=>[...e]))}const mn=(t,e)=>(t+e)/2;function pn(t,e,n=mn){const o=Array.from({length:t.vertexCount},(c,l)=>({direction:[t.directions[l*3]??0,t.directions[l*3+1]??0,t.directions[l*3+2]??0],radius:t.radii[l]??1,surface:t.surface[l]??0})),a=new Map,s=(c,l)=>{const i=c<l?`${c}:${l}`:`${l}:${c}`,d=a.get(i);if(d!==void 0)return d;const u=o[c],p=o[l],[w,g,k]=u.direction,[x,M,A]=p.direction,D=Math.hypot(w*u.radius-x*p.radius,g*u.radius-M*p.radius,k*u.radius-A*p.radius),[R,m,v]=[(w+x)/2,(g+M)/2,(k+A)/2],y=Math.hypot(R,m,v)||1,T=n(u.surface,p.surface);o.push({direction:[R/y,m/y,v/y],radius:(u.radius+p.radius)/2+e(D),surface:T});const F=o.length-1;return a.set(i,F),F},r=[];for(let c=0;c<t.faceCount;c+=1){const l=t.faces[c*3],i=t.faces[c*3+1],d=t.faces[c*3+2],u=s(l,i),p=s(i,d),w=s(d,l);r.push([l,u,w],[i,p,u],[d,w,p],[u,p,w])}return mt(o,r)}function mt(t,e){const n=new Float32Array(t.length*3),o=new Float32Array(t.length),a=new Float32Array(t.length);t.forEach((r,c)=>{n[c*3]=r.direction[0],n[c*3+1]=r.direction[1],n[c*3+2]=r.direction[2],o[c]=r.radius,a[c]=r.surface});const s=new Uint32Array(e.length*3);return e.forEach(([r,c,l],i)=>{s[i*3]=r,s[i*3+1]=c,s[i*3+2]=l}),{directions:n,radii:o,surface:a,faces:s,faceCount:e.length,vertexCount:t.length}}function fn(t){let e=t>>>0;return()=>{e=e+1831565813>>>0;let n=Math.imul(e^e>>>15,1|e);return n=n+Math.imul(n^n>>>7,61|n)^n,((n^n>>>14)>>>0)/4294967296}}function gn(t){const e=un();return{seed:t,mesh:e,temperature:new Float32Array(e.vertexCount),faceColour:new Uint8ClampedArray(e.faceCount*3),seaRadius:0}}function yn(t,e){return{...t,mesh:e,temperature:new Float32Array(e.vertexCount),faceColour:new Uint8ClampedArray(e.faceCount*3)}}function wn(t,e){return Math.abs(t.mesh.directions[e*3+1]??0)}function pt(t,e,n){const o=t.mesh.faces[n*3]??0,a=t.mesh.faces[n*3+1]??0,s=t.mesh.faces[n*3+2]??0;return((e[o]??0)+(e[a]??0)+(e[s]??0))/3}function vn(t,e){return pt(t,t.mesh.radii,e)}const ft=(t=4,e=.28,n=.2)=>o=>{const a=fn(o.seed);let s=o.mesh;const r=Float32Array.from(s.surface,()=>a());s={...s,surface:r};for(let c=0;c<t;c+=1)s=pn(s,l=>l*e*(a()-.5),(l,i)=>{const d=.5+(a()-.5)*(l-i)*n;return Math.min(1,Math.max(0,l*(1-d)+i*d))});return yn(o,s)},gt=({equator:t=1,pole:e=.05,peak:n=0}={})=>o=>{const a=new Float32Array(o.mesh.vertexCount),s=o.mesh.radii,r=s.reduce((i,d)=>Math.min(i,d),1/0),l=s.reduce((i,d)=>Math.max(i,d),-1/0)-r||1;for(let i=0;i<o.mesh.vertexCount;i+=1){const d=((s[i]??1)-r)/l,u=wn(o,i)**2.2;a[i]=t+(e-t)*u+(n-t)*d}return{...o,temperature:a}},yt=(t=.55)=>e=>{const n=Float32Array.from(e.mesh.radii).sort(),o=Math.min(n.length-1,Math.floor(n.length*t)),a=n[o]??1,s=Float32Array.from(e.mesh.radii,r=>Math.max(r,a));return{...e,mesh:{...e.mesh,radii:s},seaRadius:a}},bn=[24,92,168],kn=[62,176,206],xn=[214,196,138],Ge=[190,158,84],de=[70,138,66],Tn=[74,104,76],Sn=[136,128,116],Ke=[238,243,247];function j(t,e,n){const o=Math.min(1,Math.max(0,n));return[t[0]+(e[0]-t[0])*o,t[1]+(e[1]-t[1])*o,t[2]+(e[2]-t[2])*o]}function $n(t){return t>.78?Ge:t>.62?j(de,Ge,(t-.62)/.16):t>.3?de:j(Tn,de,(t-.12)*5.5)}const wt=t=>{const e=new Uint8ClampedArray(t.mesh.faceCount*3),n=t.mesh.radii.reduce((a,s)=>Math.max(a,s),-1/0),o=Math.max(1e-6,n-t.seaRadius);for(let a=0;a<t.mesh.faceCount;a+=1){const s=(vn(t,a)-t.seaRadius)/o,r=pt(t,t.temperature,a);let c;s<=.002?(c=j(kn,bn,.55),r<.16&&(c=j(c,Ke,(.16-r)*6))):(c=j(xn,$n(r),Math.min(1,s*9)),c=j(c,Sn,Math.max(0,s-.55)*2.2),r<.26&&(c=j(c,Ke,(.26-r)*4))),e[a*3]=c[0],e[a*3+1]=c[1],e[a*3+2]=c[2]}return{...t,faceColour:e}},Mn=[ft(),yt(),gt(),wt];function An(t,e=Mn){return e.reduce((n,o)=>o(n),gn(t))}function vt(t){return An(t.seed,[ft(t.levels,t.roughness),yt(t.share),gt(),wt])}const Ye=.3,In=[-.5,.45,.74],En=1.02;class xe{size;pixels;depth;view=new Float32Array(0);screen=new Float32Array(0);constructor(e,n=new Uint8ClampedArray(e*e*4)){if(n.length!==e*e*4)throw new Error(`SphereRaster: ${e}×${e} needs ${e*e*4} bytes, not ${n.length}`);this.size=e,this.pixels=n,this.depth=new Float32Array(e*e)}paint(e,n){const{size:o,pixels:a,depth:s}=this;a.fill(0),s.fill(-1/0);const[r,c,l]=Cn(n.light??In),i=n.tilt??-.38,d=Math.cos(i),u=Math.sin(i),p=Math.cos(n.rotation),w=Math.sin(n.rotation),{directions:g,radii:k,faces:x,faceCount:M,vertexCount:A}=e.mesh;let D=1;for(let y=0;y<A;y+=1){const T=k[y]??1;T>D&&(D=T)}const R=o/(2*D*En);this.view.length<A*3&&(this.view=new Float32Array(A*3),this.screen=new Float32Array(A*3));const m=this.view,v=this.screen;for(let y=0;y<A;y+=1){const T=k[y]??1,F=(g[y*3]??0)*T,C=(g[y*3+1]??0)*T,b=(g[y*3+2]??0)*T,f=F*p-b*w,$=F*w+b*p,S=C*d+$*u,E=-C*u+$*d;m[y*3]=f,m[y*3+1]=S,m[y*3+2]=E,v[y*3]=o/2+f*R,v[y*3+1]=o/2-S*R,v[y*3+2]=E}for(let y=0;y<M;y+=1){const T=x[y*3]??0,F=x[y*3+1]??0,C=x[y*3+2]??0,b=v[T*3],f=v[T*3+1],$=v[T*3+2],S=v[F*3],E=v[F*3+1],L=v[F*3+2],P=v[C*3],W=v[C*3+1],G=v[C*3+2],se=(S-b)*(W-f)-(E-f)*(P-b);if(se>=0)continue;const Te=m[T*3],Se=m[T*3+1],$e=m[T*3+2],Me=m[F*3]-Te,Ae=m[F*3+1]-Se,Ie=m[F*3+2]-$e,Ee=m[C*3]-Te,Ce=m[C*3+1]-Se,De=m[C*3+2]-$e,Re=Ae*De-Ie*Ce,Pe=Ie*Ee-Me*De,Fe=Me*Ce-Ae*Ee,re=Math.hypot(Re,Pe,Fe)||1,It=Re/re*r+Pe/re*c+Fe/re*l,ie=Ye+(1-Ye)*Math.max(0,It),Et=(e.faceColour[y*3]??0)*ie,Ct=(e.faceColour[y*3+1]??0)*ie,Dt=(e.faceColour[y*3+2]??0)*ie,Rt=Math.max(0,Math.floor(Math.min(b,S,P))),Pt=Math.min(o-1,Math.ceil(Math.max(b,S,P))),Ft=Math.max(0,Math.floor(Math.min(f,E,W))),Ot=Math.min(o-1,Math.ceil(Math.max(f,E,W)));for(let V=Ft;V<=Ot;V+=1)for(let X=Rt;X<=Pt;X+=1){const ce=X+.5,le=V+.5,Wt=(S-b)*(le-f)-(E-f)*(ce-b),Oe=(P-S)*(le-E)-(W-E)*(ce-S),We=(b-P)*(le-W)-(f-W)*(ce-P);if(Wt>0||Oe>0||We>0)continue;const Ne=Oe/se,Be=We/se,Le=$*Ne+L*Be+G*(1-Ne-Be),J=V*o+X;Le<=s[J]||(s[J]=Le,a[J*4]=Et,a[J*4+1]=Ct,a[J*4+2]=Dt,a[J*4+3]=255)}}return a}}function Cn([t,e,n]){const o=Math.hypot(t,e,n)||1;return[t/o,e/o,n/o]}const ve={levels:4,roughness:.28,share:.55},U=32;let ue=null,Ue=null,me=null;function ze(t,e){const n=document.querySelector('link[rel="icon"]');if(!n)return;ue??=Object.assign(document.createElement("canvas"),{width:U,height:U});const o=ue.getContext("2d");o&&(me??=o.createImageData(U,U),Ue??=new xe(U,me.data),Ue.paint(t,{rotation:e}),o.putImageData(me,0,0),n.type="image/png",n.href=ue.toDataURL("image/png"))}function bt(t){let e=!0;if(typeof IntersectionObserver!="function")return{onScreen:()=>e,stop:()=>{}};const n=new IntersectionObserver(o=>{for(const a of o)e=a.isIntersecting},{rootMargin:"100px"});return n.observe(t),{onScreen:()=>e,stop:()=>n.disconnect()}}const Dn=90,Rn=400,pe=new WeakMap;function be(t,e){pe.get(t)?.();const n=t.getContext("2d");if(!n)return()=>{};const o=e??{...ve,seed:Math.floor(Math.random()*16777215)},a=t.width,s=vt(o),r=n.createImageData(a,a),c=new xe(a,r.data);t.dataset.seed=String(o.seed),t.title=`World ${o.seed}, ${s.mesh.faceCount.toLocaleString("en")} triangles`;const l=k=>{c.paint(s,{rotation:k}),n.putImageData(r,0,0),t.classList.add("grown")};if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return l(.6),ze(s,.6),pe.set(t,()=>{}),()=>{};let i=0,d=-1/0;const u=performance.now(),p=bt(t),w=k=>{const x=(k-u)/1e3/Dn*Math.PI*2;p.onScreen()&&l(x),k-d>Rn&&(ze(s,x),d=k),i=requestAnimationFrame(w)};i=requestAnimationFrame(w);const g=()=>{cancelAnimationFrame(i),p.stop()};return pe.set(t,g),g}function Pn(){const t=document.querySelector("canvas.planet");return t?be(t,ut()??void 0):()=>{}}const q=360,Fn=60,On=1.4,Ve=Math.PI*2/Fn,Xe=Math.PI*4;function Wn(t){const e=h("canvas",{class:"world",width:q,height:q}),n=e.getContext("2d");if(!n)return()=>{};const o={...ve,seed:Math.floor(Math.random()*16777215)},a=n.createImageData(q,q),s=new xe(q,a.data),r=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let c,l=.6,i=-.38,d=!r,u=null,p=0,w=performance.now();const g=h("p",{class:"hint"}),k=document.querySelector("canvas.planet"),x=(20*4**ve.levels).toLocaleString("en"),M=()=>{c=vt(o);const f=ut();g.textContent=`World ${o.seed}: ${c.mesh.faceCount.toLocaleString("en")} triangles. `+(f?`The header is keeping world ${f.seed}, ${(20*4**f.levels).toLocaleString("en")} triangles.`:`The header grows a new one every visit, ${x} triangles each.`),D.hidden=!f,R()},A=h("button",{type:"button",onclick:()=>{cn({...o}),k&&be(k,{...o}),M()}},"Put it in the header"),D=h("button",{type:"button",hidden:!0,onclick:()=>{ln(),k&&be(k),M()}},"Let the header grow its own"),R=()=>{s.paint(c,{rotation:l,tilt:i}),n.putImageData(a,0,0)};let m=0;const v=bt(e),y=f=>{const $=Math.min(.1,(f-w)/1e3);if(!u&&v.onScreen()){if(p!==0){p*=Math.exp(-$/On);const S=d?Ve:0;(Math.abs(p)<=S||Math.abs(p)<.01)&&(p=0)}p!==0?(l-=p*$,R()):d&&(l+=Ve*$,R()),ye.send({byRadians:p*$,tiltedBy:0,seconds:$})}w=f,m=requestAnimationFrame(y)};e.addEventListener("pointerdown",f=>{u={x:f.clientX,y:f.clientY,at:f.timeStamp},p=0,e.setPointerCapture(f.pointerId)}),e.addEventListener("pointermove",f=>{if(!u)return;const $=e.clientWidth||q,S=(f.clientX-u.x)/$*Math.PI;l-=S;const E=i;i=Math.max(-1.2,Math.min(1.2,i-(f.clientY-u.y)/$*Math.PI)),ye.send({byRadians:S,tiltedBy:i-E,seconds:0});const L=Math.max(.004,(f.timeStamp-u.at)/1e3);p=Math.max(-Xe,Math.min(Xe,p*.4+S/L*.6)),u={x:f.clientX,y:f.clientY,at:f.timeStamp},R()}),e.addEventListener("pointerup",f=>{u&&f.timeStamp-u.at>120&&(p=0),u=null,w=performance.now()}),e.addEventListener("pointercancel",()=>{u=null,p=0});const T=h("input",{type:"number",min:0,value:o.seed,onchange:()=>{o.seed=Math.max(0,Math.floor(Number(T.value)||0)),M()}}),F=h("button",{type:"button",onclick:()=>{o.seed=Math.floor(Math.random()*16777215),T.value=String(o.seed),M()}},"Another world"),C=h("button",{type:"button",onclick:()=>{d=!d,C.textContent=d?"Hold still":"Turn"}},d?"Hold still":"Turn"),b=(f,$,S,E,L,P)=>{const W=h("output",{},P(o[f])),G=h("input",{type:"range",min:S,max:E,step:L,value:o[f],onchange:()=>{o[f]=Number(G.value),W.textContent=P(o[f]),M()},oninput:()=>{W.textContent=P(Number(G.value))}});return h("label",{},`${$}: `,W,G)};return t.append(e,h("div",{class:"row"},h("span",{},"Seed "),T,F,C,A,D),h("div",{class:"dials"},b("levels","Detail",2,6,1,f=>`${f} splits`),b("roughness","Roughness",.02,1,.01,f=>f.toFixed(2)),b("share","Sea",0,.98,.01,f=>`${Math.round(f*100)}%`)),g),M(),m=requestAnimationFrame(y),()=>{cancelAnimationFrame(m),v.stop()}}const Nn={name:"world",apps:{worlds:Wn},install:()=>Pn()},z=[Nn,rn,Yt,Xt,Ht];function Ze(t){const e=[];for(const n of document.querySelectorAll(".app[data-app]")){const o=t[n.dataset.app??""]?.(n);o&&e.push(o)}return()=>{for(const n of e)n()}}function Bn(t){const e={},n=t.fields.theme;(n==="dark"||n==="light")&&(e["data-page-theme"]=n);const o=t.fields.sky;return o&&(e["data-sky"]=o),e}const Ln=["data-page-theme","data-sky"];function jn(t,e){const o=/^https?:/.test(e)?' target="_blank" rel="noopener noreferrer"':"";return`<a href="${O(e)}"${o}>${t}</a>`}const Hn=["large","wide"];function _n(t,e,n){const o=n&&Hn.includes(n)?` class="${n}"`:"";return`<img src="${O(e)}" alt="${O(t)}"${o}>`}const Jn=/(`[^`]+`|!\[[^\]]*\]\([^)\s]+(?:\s+"[^"]*")?\)|\[[^\]]+\]\([^)\s]+\))/g,qn=/^!\[([^\]]*)\]\(([^)\s]+)(?:\s+"([^"]*)")?\)$/,Gn=/^\[([^\]]+)\]\(([^)\s]+)\)$/;function kt(t){return t.split(Jn).map(e=>{if(e.startsWith("`")&&e.endsWith("`")&&e.length>1)return`<code>${O(e.slice(1,-1))}</code>`;const n=qn.exec(e);if(n)return _n(n[1]??"",n[2]??"",n[3]);const o=Gn.exec(e);return o?jn(kt(o[1]??""),o[2]??""):O(e)}).join("")}function Kn(t){return t.split(/(<code>[\s\S]*?<\/code>)/g).map(e=>e.startsWith("<code>")?e:e.replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/(^|[^*])\*([^*]+)\*/g,"$1<em>$2</em>").replace(/ {2,}\n/g,"<br>").replace(/\n/g," ").replace(/ -- /g," — ")).join("")}function _(t){return Kn(kt(t))}const Qe=/^(?:[-*]|\d+\.)\s/;function Yn(t,e,n){if(!Qe.test(t[0]??""))return!1;const o=e.slice(n).find(a=>a.trim()!=="");return o!==void 0&&Qe.test(o)}function Un(t){const e=[],n=t.replace(/\r\n?/g,`
`).split(`
...
`);return`<h${o} id="${zn(a)}">${_(a)}</h${o}>`}function Xn(t){if(!t[0]?.startsWith("```"))return null;const e=t.slice(1,-1).join(`
`);return`<pre><code>${O(e)}</code></pre>`}function Zn(t,e){const n=[];for(const o of t)e.test(o)?n.push(o.replace(e,"")):n.length&&(n[n.length-1]+=`
${o.trim()}`);return n}function Qn(t){const e=t[0]??"",n=/^\d+\.\s/.test(e),o=/^[-*]\s/.test(e);if(!n&&!o)return null;const a=n?/^\d+\.\s+/:/^[-*]\s+/;if(!t.every(c=>a.test(c)||/^\s/.test(c)))return null;const s=n?"ol":"ul",r=Zn(t,a).map(c=>`<li>${_(c)}</li>`).join("");return`<${s}>${r}</${s}>`}function eo(t){return t.every(n=>n.includes(" :: "))?`<dl>${t.map(n=>{const o=n.indexOf(" :: ");return[n.slice(0,o),n.slice(o+4)]}).map(([n,o])=>`<dt>${_(n)}</dt><dd>${_(o)}</dd>`).join("")}</dl>`:null}function to(t){if(!t.every(n=>n.startsWith(">")))return null;const e=t.map(n=>n.replace(/^>\s?/,"")).join(" ");return`<blockquote>${_(e)}</blockquote>`}function no(t){const e=/^::([a-z0-9-]+)$/.exec(t[0]??"");return!e||t.length!==1?null:`<div class="app" data-app="${e[1]}"></div>`}function oo(t){return t.length===1&&/^-{3,}$/.test(t[0]??"")?"<hr>":null}function ao(t){const e=t.length===1&&/^(\\+)$/.exec(t[0]??"");return e?`<div class="space" style="--n:${e[1]?.length??1}"></div>`:null}function so(t){return t.length===1&&/^!\[[^\]]*\]\([^)\s]+(?:\s+"[^"]*")?\)$/.test(t[0]??"")?`<figure>${_(t[0]??"")}</figure>`:null}function ro(t){return`<p>${_(t.join(`
...
`).trim()}}function Ro(t){const n=t.replace(/\.md$/,"").replace(/(^|\/)index$/,"");return n===""?"/":`/${n}/`}function Po(t){if(t==="/")return null;const e=t.slice(0,-1);return e.slice(0,e.lastIndexOf("/")+1)}function ot(t){if(t==="/")return"/";const e=t.slice(0,-1);return e.slice(e.lastIndexOf("/")+1)}function Fo(t){const{fields:e,body:n}=Do(t.markdown),o=Ro(t.file);return{file:t.file,route:o,parent:Po(o),name:ot(o),title:e.title??ot(o),summary:e.summary??"",order:Number(e.order??"100"),body:n,fields:e}}function at(t,e){return t.order-e.order||t.name.localeCompare(e.name)}class Oo{byRoute;constructor(e){const n=e.map(Fo).sort(at);this.byRoute=new Map(n.map(o=>[o.route,o]))}get pages(){return[...this.byRoute.values()]}at(e){return this.byRoute.get(e)}childrenOf(e){return this.pages.filter(n=>n.parent===e).sort(at)}trailTo(e){const n=this.at(e);return n?n.parent===null?[n]:[...this.trailTo(n.parent),n]:[]}}const fe=new Oo(Eo);function st(){const t=[...$t,...z.flatMap(i=>i.commands??[])],e=Object.assign({},...z.map(i=>i.apps??{})),o=(i=>i.endsWith("/")?i:`${i}/`)(window.location.pathname),a=fe.at(o);let s=Ze(e),r=null;const c=uo(fe,i=>{s(),s=Ze(e);for(const d of z)d.arrive?.(i);r?.moveTo(i.route)});if(r=Io(fe,a?o:"/",{navigate:c,commands:t}),a)for(const i of z)i.arrive?.(a);const l={run:i=>r?.run(i)};for(const i of z)i.install?.(l)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",st):st();
